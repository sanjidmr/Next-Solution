...
import { 
  ...
  Search as SearchIcon, Eye as EyeIcon, ArrowUpRight as ArrowUpRightIcon, Cpu as CpuIcon
} from 'lucide-react';
...
export default function AdminPanel({ currentLang }: AdminPanelProps) {
  ...
  // Tab State
  const [activeSubTab, setActiveSubTab] = useState<'overview' | 'messages' | 'subscribers' | 'blogs' | 'services' | 'portfolio' | 'pricing' | 'faqs' | 'testimonials' | 'team' | 'media' | 'settings' | 'users' | 'legal' | 'whychooseus' | 'process' | 'techstack'>('overview');
  ...
  if (!isAuthenticated) {
    return (
      <section id="admin-auth-screen" className="flex items-center justify-center min-h-[70vh] bg-white px-4 py-16">
        <div className="w-full max-w-md rounded-2xl border border-gray-100 bg-white p-6 md:p-8 shadow-xl text-center space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4 text-left">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-blue-500 transition"
              />
            </div>

            {authError && (
              <span className="block text-xs text-red-500 font-semibold">{authError}</span>
            )}

            <button
              id="admin-login-btn"
              type="submit"
              className="w-full rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold py-3 transition shadow-sm"
            >
              {currentLang === 'en' ? 'Unlock Control Panel' : 'প্যানেল আনলক করুন'}
            </button>
          </form>

          <div className="rounded bg-blue-50/50 border border-blue-100 p-3 text-[10px] text-gray-500 max-w-xs mx-auto">
  ...
  return (
    <section id="admin-main-dashboard" className="bg-white py-12 min-h-screen">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-6 mb-8">
          <div>
            <h2 className="text-xl font-black text-gray-900 flex items-center gap-2">
              <SparklesIcon className="h-5.5 w-5.5 text-blue-500" />
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="rounded-lg bg-gray-100 text-gray-600 hover:text-gray-900 hover:bg-gray-200 px-3 py-1.5 text-xs font-bold transition flex items-center gap-1.5"
            >
            ...
            <button
              ...
              className="rounded-lg border border-red-200 bg-red-50/50 text-red-600 hover:bg-red-100 px-3 py-1.5 text-xs font-bold transition flex items-center gap-1.5"
            >
        ...
        {successNotice && (
          <div id="admin-toast-banner" className="mb-6 rounded-xl bg-emerald-50 border border-emerald-200 p-4 text-xs font-bold text-emerald-700 flex items-center space-x-2">
        ...
        <div id="admin-metrics-grid" className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
            ...
            <span className="text-lg font-bold text-gray-900 block mt-1.5">{messages.length} Total</span>
            <span className="text-[10px] text-gray-400">{messages.filter(m => m.status === 'unread').length} Unread</span>
          </div>
          <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
            <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1">
            ...
            <span className="text-lg font-bold text-gray-900 block mt-1.5">{subscribers.length} Members</span>
            <span className="text-[10px] text-gray-400">Live persistence</span>
          </div>
          <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
            <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1">
            ...
            <span className="text-lg font-bold text-gray-900 block mt-1.5">{blogs.filter(b => b.status === 'published').length} Live</span>
            <span className="text-[10px] text-gray-400">{blogs.filter(b => b.status === 'draft').length} Drafts</span>
          </div>
          <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
            <span className="block text-[9px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1">
              <SettingsIcon className="h-3.5 w-3.5 text-blue-500" />
              <span>Core Services</span>
            </span>
            <span className="text-lg font-bold text-gray-900 block mt-1.5">{services.length} Capabilities</span>
            <span className="text-[10px] text-gray-400">Dual translations</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-3 rounded-xl border border-gray-100 bg-white p-3 flex flex-row lg:flex-col gap-1 overflow-x-auto lg:overflow-x-visible shadow-sm">
            {[
          ...
          <div className="lg:col-span-9 bg-white border border-gray-100 rounded-2xl p-5 md:p-6 shadow-sm overflow-hidden">
              ...
              <div id="panel-overview-desk" className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-3">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest font-mono">Live Systems Connected</span>
                  </div>
                </div>

                {/* KPI Metrics row */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="rounded-xl border border-gray-100 bg-white p-4 space-y-1 shadow-sm">
                    <span className="text-[10px] text-gray-400 font-bold uppercase block">Monthly Revenue</span>
                    <span className="text-xl font-bold text-gray-900 font-mono">$24,500</span>
                    <span className="text-[9px] text-emerald-500 font-bold flex items-center">↑ 12.4% vs last month</span>
                  </div>
                  <div className="rounded-xl border border-gray-100 bg-white p-4 space-y-1 shadow-sm">
                    <span className="text-[10px] text-gray-400 font-bold uppercase block">Lead Conversion</span>
                    <span className="text-xl font-bold text-gray-900 font-mono">8.42%</span>
                    <span className="text-[9px] text-emerald-500 font-bold flex items-center">↑ 2.1% conversion rate</span>
                  </div>
                  <div className="rounded-xl border border-gray-100 bg-white p-4 space-y-1 shadow-sm">
                    <span className="text-[10px] text-gray-400 font-bold uppercase block">Total Leads Logged</span>
                    <span className="text-xl font-bold text-gray-900 font-mono">{messages.length}</span>
                    <span className="text-[9px] text-blue-500 font-semibold block">Real-time DB synced</span>
                  </div>
                  <div className="rounded-xl border border-gray-100 bg-white p-4 space-y-1 shadow-sm">
                    <span className="text-[10px] text-gray-400 font-bold uppercase block">Subscriber Intel</span>
                    <span className="text-xl font-bold text-gray-900 font-mono">{subscribers.length}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                  <div className="md:col-span-8 rounded-xl border border-gray-100 bg-white p-4 md:p-5 space-y-4 shadow-sm">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="text-xs font-bold text-gray-900 uppercase tracking-wide">Client Traffic & Lead Volume</h4>
                        <p className="text-[10px] text-gray-400 mt-0.5">Bi-weekly analytics aggregation</p>
                      </div>
                      <select className="text-[10px] bg-gray-50 border border-gray-100 rounded px-2 py-1 font-bold">
                    ...
                    <div className="relative h-48 w-full bg-gray-50/50 rounded-lg flex items-end px-4 pb-2 pt-6">
                      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                        ...
                        {/* Main line path */}
                        <path d="M 0 85 Q 15 65 30 75 T 60 40 T 90 20 T 100 15" fill="none" stroke="#2563eb" strokeWidth="2" strokeLinecap="round" />
                        {/* Dots */}
                        <circle cx="30" cy="75" r="2" fill="#2563eb" />
                        <circle cx="60" cy="40" r="2" fill="#2563eb" />
                        <circle cx="90" cy="20" r="2" fill="#2563eb" />
                      </svg>
                      {/* X Axis Labels */}
                      <div className="w-full flex justify-between text-[8px] font-bold text-gray-400 font-mono relative z-10">
                        <span>Mon</span>
                        <span>Wed</span>
                        <span>Fri</span>
                        <span>Sun</span>
                  ...
                  <div className="md:col-span-4 rounded-xl border border-gray-100 bg-white p-4 space-y-4 shadow-sm">
                    <div>
                      <h4 className="text-xs font-bold text-gray-900 uppercase tracking-wide">Acquisition Channels</h4>
                      <p className="text-[10px] text-gray-400 mt-0.5">Where inbound leads originate</p>
                    </div>
                    {/* Funnel list representation */}
                    <div className="space-y-3 pt-2">
                      {[
                        { label: 'Organic Search (SEO)', pct: '48%', color: 'bg-blue-600' },
                ...
                <div className="rounded-xl border border-gray-100 bg-white p-4 space-y-3 shadow-sm">
                  <h4 className="text-xs font-bold text-gray-900 uppercase tracking-wide">System Audit & Status Log</h4>
                  <div className="divide-y divide-gray-50 text-[10px]">
                    <div className="py-2.5 flex items-center justify-between text-gray-600">
                      ...
                      <span className="font-mono text-gray-400">July 10, 2026</span>
                    </div>
                    <div className="py-2.5 flex items-center justify-between text-gray-600">
                    ...
                    <div className="py-2.5 flex items-center justify-between text-gray-600">
                      ...
                      <span className="font-mono text-blue-500 font-semibold">Active</span>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-50 pb-2">
                        <div>
                          ...
                          <span className="text-gray-400 font-mono text-[10px]">{msg.email} | {msg.phone}</span>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-[11px] font-semibold text-gray-500 bg-gray-50 p-2.5 rounded-lg">
                        <div>Desired Service: <span className="text-gray-800">{msg.service}</span></div>
                        <div>Estimated Budget: <span className="text-gray-800">{msg.budget}</span></div>
                      </div>

                      <p className="text-gray-700 leading-relaxed text-[11px] whitespace-pre-line bg-gray-50 p-2.5 rounded-lg">
                        {msg.message}
                      </p>
              ...
              <div id="panel-subs-desk" className="space-y-6">
                ...
                <div className="overflow-x-auto border border-gray-100 rounded-xl">
                  <table className="w-full text-left text-xs border-collapse">
                     <thead>
                       <tr className="bg-gray-50 border-b border-gray-100 font-bold uppercase tracking-wider text-gray-400">
                ...
                <div className="flex items-center justify-between border-b border-gray-50 pb-3">
                  ...
                  {!isCreatingBlog && (
                    <button
                      ...
                      className="rounded bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-3 py-1.5 flex items-center gap-1 transition shadow-sm"
                    >
                  ...
                  <form onSubmit={handleSaveBlog} className="space-y-4 text-xs">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Title (English)</label>
                        <input
                          ...
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Title (Bangla)</label>
                        <input
                          id="blog-form-titleBn"
                          type="text"
                          required
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Category (English)</label>
                        <input
                          id="blog-form-catEn"
                          type="text"
                          required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Category (Bangla)</label>
                        <input
                          id="blog-form-catBn"
                          type="text"
                          required
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Excerpt (English)</label>
                        <textarea
                          id="blog-form-exEn"
                          rows={2}
                          required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Excerpt (Bangla)</label>
                        <textarea
                          id="blog-form-exBn"
                          rows={2}
                          required
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Content (English)</label>
                        <textarea
                          id="blog-form-conEn"
                          rows={6}
                          required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Content (Bangla)</label>
                        <textarea
                          id="blog-form-conBn"
                          rows={6}
                          required
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Author</label>
                        <input
                          id="blog-form-author"
                          type="text"
                          required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Image URL</label>
                        <input
                          id="blog-form-image"
                          type="text"
                          required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Publication Status</label>
                        <select
                          id="blog-form-status"
                    ...
                    <div className="flex justify-end space-x-2 pt-2">
                      <button
                        ...
                        className="rounded border border-gray-200 px-4 py-2 hover:bg-gray-50"
                      >
                        Cancel
                      </button>
                      <button
                        id="blog-submit-btn"
                        type="submit"
                        className="rounded bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 font-bold shadow-sm"
                      >
                        Save & Deploy
                      </button>
                    </div>
                      ...
                      <tbody className="divide-y divide-gray-100 text-gray-600">
                        {blogs.map((b) => (
                          <tr key={b.id} className="hover:bg-gray-50">
                            <td className="p-4 font-semibold text-gray-900 max-w-xs truncate">{b.titleEn}</td>
                            <td className="p-4">{b.author}</td>
                            <td className="p-4">{b.categoryEn}</td>
                            <td className="p-4">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                                b.status === 'published' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-600'
                              }`}>
                                {b.status}
                              </span>
                            </td>
            ...
            {activeSubTab === 'services' && (
              <div id="panel-services-desk" className="space-y-6">
                <div className="flex items-center justify-between border-b border-gray-50 pb-3">
                  ...
                  {!editingService && (
                    <button
                      ...
                      className="rounded bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 text-xs font-bold transition flex items-center gap-1 shadow-sm"
                    >
                      <PlusIcon className="h-3.5 w-3.5" />
                ...
                {editingService ? (
                  <form onSubmit={handleSaveServiceEdit} className="space-y-6 text-xs bg-gray-50/50 p-5 rounded-2xl border border-gray-100 max-h-[80vh] overflow-y-auto">
                    <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                      <h4 className="text-sm font-bold text-blue-600">Editing: {editingService.titleEn}</h4>
                      <span className="text-[10px] font-mono text-gray-400">ID: {editingService.id}</span>
                    </div>
                    
                    {/* SECTION 1: Core Details */}
                    <div className="space-y-4">
                      <h5 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">1. Core Details</h5>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Starting Price</label>
                          <input
                            ...
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Category Group</label>
                          <input
                            id="service-form-cat"
                            type="text"
                            required
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Slug (e.g. web-development)</label>
                          <input
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Lucide Icon Name (e.g. Sparkles, Laptop, Smartphone)</label>
                          <input
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Description (English)</label>
                          <textarea
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Description (Bangla)</label>
                          <textarea
                            id="service-form-descBn"
                            rows={3}
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-100">
                      <h5 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">2. High-Impact Copywriting</h5>
                      
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Subtitle (English)</label>
                          <input
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Subtitle (Bangla)</label>
                          <input
                            id="service-form-subBn"
                            type="text"
                            value={editingService.subtitleBn || ''}
                            onChange={(e) => setEditingService({ ...editingService, subtitleBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                            placeholder="হুক লাইন বাংলায়"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Why You Need This (English)</label>
                          <textarea
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Why You Need This (Bangla)</label>
                          <textarea
                            id="service-form-whyBn"
                            rows={2}
                            value={editingService.whyNeedBn || ''}
                            onChange={(e) => setEditingService({ ...editingService, whyNeedBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                            placeholder="পেইন পয়েন্ট বাংলায়"
                          ></textarea>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Who Is This For? (English)</label>
                          <textarea
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Who Is This For? (Bangla)</label>
                          <textarea
                            id="service-form-forBn"
                            rows={2}
                            value={editingService.whoForBn || ''}
                            onChange={(e) => setEditingService({ ...editingService, whoForBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                            placeholder="টার্গেট কাস্টমার বাংলায়"
                          ></textarea>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Business Impact (English)</label>
                          <textarea
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Business Impact (Bangla)</label>
                          <textarea
                            id="service-form-impactBn"
                            rows={2}
                            value={editingService.businessImpactBn || ''}
                            onChange={(e) => setEditingService({ ...editingService, businessImpactBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                            placeholder="ব্যবসায়িক প্রভাব বাংলায়"
                          ></textarea>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-100">
                      <h5 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">3. Arrays & Technology Stack</h5>
                      
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Technology Ecosystem (Comma-separated)</label>
                        <input
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Core Deliverables (En, Comma-separated)</label>
                          <textarea
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Core Deliverables (Bn, Comma-separated)</label>
                          <textarea
                            id="service-form-featBn"
                            rows={2}
                            value={editingService.featuresBn ? editingService.featuresBn.join(', ') : ''}
                            onChange={(e) => setEditingService({ ...editingService, featuresBn: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                            placeholder="ডেলিভারি ১, ডেলিভারি ২"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Strategic Benefits (En, Comma-separated)</label>
                          <textarea
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Strategic Benefits (Bn, Comma-separated)</label>
                          <textarea
                            id="service-form-benBn"
                            rows={2}
                            value={editingService.benefitsBn ? editingService.benefitsBn.join(', ') : ''}
                            onChange={(e) => setEditingService({ ...editingService, benefitsBn: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                            placeholder="উপকারিতা ১, উপকারিতা ২"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Execution Framework (En, One step per line)</label>
                          <textarea
                            ...
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 font-mono text-[10px] focus:outline-none focus:border-blue-500"
                            placeholder="Step 1&#10;Step 2&#10;Step 3"
                          />
                        </div>
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Execution Framework (Bn, One step per line)</label>
                          <textarea
                            id="service-form-procBn"
                            rows={3}
                            value={editingService.processBn ? editingService.processBn.join('\n') : ''}
                            onChange={(e) => setEditingService({ ...editingService, processBn: e.target.value.split('\n').map(s => s.trim()).filter(Boolean) })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 font-mono text-[10px] focus:outline-none focus:border-blue-500"
                            placeholder="ধাপ ১&#10;ধাপ ২&#10;ধাপ ৩"
                          />
                        </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-100">
                      <h5 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">4. Structured JSON Modules (Dynamic Lists)</h5>
                      
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Sub-Services List JSON</label>
                        <textarea
                          ...
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 font-mono text-[10px] focus:outline-none focus:border-blue-500"
                        />
                        <span className="text-[9px] text-gray-400 block font-mono">{'Format: [{"titleEn": "...", "titleBn": "...", "descEn": "...", "descBn": "..."}]'}</span>
                      </div>

                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Pricing Tiers JSON</label>
                        <textarea
                          ...
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 font-mono text-[10px] focus:outline-none focus:border-blue-500"
                        />
                        <span className="text-[9px] text-gray-400 block font-mono">{'Format: [{"nameEn": "...", "nameBn": "...", "price": "...", "periodEn": "...", "periodBn": "...", "featuresEn": ["A"], "featuresBn": ["১"]}]'}</span>
                      </div>

                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Service FAQs JSON</label>
                        <textarea
                          ...
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 font-mono text-[10px] focus:outline-none focus:border-blue-500"
                        />
                        <span className="text-[9px] text-gray-400 block font-mono">{'Format: [{"questionEn": "...", "questionBn": "...", "answerEn": "...", "answerBn": "..."}]'}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end space-x-2 pt-4 border-t border-gray-100">
                      <button
                        ...
                        className="rounded border border-gray-200 px-4 py-2 hover:bg-gray-50 font-bold"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        type="submit"
                        className="rounded bg-blue-600 text-white px-5 py-2 font-bold shadow-sm hover:bg-blue-700"
                      >
                        Save Changes
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {services.map((srv) => (
                      <div key={srv.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between text-xs hover:border-blue-500 transition shadow-sm bg-white">
                        <div>
                          <span className="font-bold text-sm text-gray-900">{srv.titleEn}</span>
                          <span className="block text-gray-400 font-mono mt-1">{srv.category} • Budget starting at: <span className="text-blue-600 font-bold">{srv.price}</span></span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <button
                            id={`edit-service-btn-${srv.id}`}
                            onClick={() => setEditingService(srv)}
                            className="rounded bg-gray-50 hover:bg-gray-100 px-3 py-1.5 font-semibold text-gray-700 transition border border-gray-100"
                          >
                            Modify details
                          </button>
                          <button
                            ...
                            className="rounded border border-red-200 bg-red-50 hover:bg-red-100 text-red-600 hover:text-red-700 p-1.5 transition"
            ...
            {activeSubTab === 'settings' && settings && (
              <form onSubmit={handleSaveSettings} className="space-y-8 text-xs">
                <h3 className="text-sm font-bold text-gray-900 border-b border-gray-50 pb-3 flex items-center justify-between">
                  <span>{currentLang === 'en' ? 'Agency Information Config' : 'এজেন্সি তথ্য কনফিগারেশন'}</span>
                  <span className="text-[10px] text-blue-600 bg-blue-50 px-2 py-0.5 rounded font-extrabold uppercase">Live DB</span>
                </h3>

                {/* Section A: Core Info */}
                <div className="space-y-4">
                  <h4 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">A. Core Contacts</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Agency Name</label>
                      <input
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Support Corporate Email</label>
                      <input
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Hotline Phone</label>
                      <input
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">GPS Office Address (English)</label>
                      <input
                ...
                <div className="space-y-4 pt-4 border-t border-gray-50">
                  <h4 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">B. Mission & Vision Statements</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Core Mission (English)</label>
                      <textarea
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Core Mission (Bengali)</label>
                      <textarea
                        id="settings-form-missionBn"
                        rows={3}
                        value={settings.aboutMissionBn || ''}
                        onChange={(e) => setSettings({ ...settings, aboutMissionBn: e.target.value })}
                        className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                        placeholder="মিশন বাংলায় লিখুন"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Long-term Vision (English)</label>
                      <textarea
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Long-term Vision (Bengali)</label>
                      <textarea
                        id="settings-form-visionBn"
                        rows={3}
                        value={settings.aboutVisionBn || ''}
                        onChange={(e) => setSettings({ ...settings, aboutVisionBn: e.target.value })}
                        className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                        placeholder="ভিশন বাংলায় লিখুন"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-4 border-t border-gray-50">
                  <h4 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">C. Statistics Counters</h4>
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Projects Done</label>
                      <input
                        ...
                        className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500 font-bold"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Happy Clients</label>
                      <input
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Team Size</label>
                      <input
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Experience Yrs</label>
                      <input
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Countries Served</label>
                      <input
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Satisfaction %</label>
                      <input
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Industries Served</label>
                      <input
                    ...
                    <div className="space-y-1.5">
                      <label className="font-semibold text-gray-500">Techs Used</label>
                      <input
                ...
                <div className="space-y-4 pt-4 border-t border-gray-50">
                  <div className="flex items-center justify-between">
                    <h4 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">D. Dynamic Lists (JSON Arrays)</h4>
                    <span className="text-[9px] text-gray-400">Validate comma structures carefully</span>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <label className="font-semibold text-gray-500">Team Members List JSON</label>
                        <span className="text-[9px] bg-gray-50 px-1.5 py-0.5 rounded text-gray-400 font-mono">
                      ...
                      <textarea
                        ...
                        className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500 font-mono text-[10px]"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <label className="font-semibold text-gray-500">Milestones Timeline JSON</label>
                        <span className="text-[9px] bg-gray-50 px-1.5 py-0.5 rounded text-gray-400 font-mono">
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <label className="font-semibold text-gray-500">Technologies Stack JSON</label>
                        <span className="text-[9px] bg-gray-50 px-1.5 py-0.5 rounded text-gray-400 font-mono">
                ...
                <div className="flex justify-end pt-4 border-t border-gray-100">
                  <button
                    ...
                    type="submit"
                    className="rounded bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 font-bold transition shadow-md shadow-blue-600/10 cursor-pointer"
                  >
                    Deploy Site Config
                  </button>
                </div>
              ...
              <div id="panel-portfolio-desk" className="space-y-6 text-xs">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 font-bold transition"
                  >
                ...
                {isCreatingPortfolio ? (
                  <form onSubmit={handleSavePortfolio} className="space-y-6 bg-gray-50/50 rounded-xl p-4 md:p-6 border border-gray-100 max-h-[80vh] overflow-y-auto text-xs">
                    
                    {/* Part 1: Project Metadata & Identity */}
                    <div className="space-y-3 p-4 rounded-xl border border-gray-100 bg-white">
                      <h4 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px] mb-2 border-b border-gray-50 pb-1.5">1. Project Identity & Metadata</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Project Title (English)</label>
                          <input
                            type="text" required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Project Title (Bengali)</label>
                          <input
                            type="text" required
                            value={portfolioForm.titleBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, titleBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                        <div className="space-y-1.5">
                          ...
                          <select
                            ...
                            className="w-full rounded border border-gray-200 bg-white px-2 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          >
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Client Name</label>
                          <input
                            type="text" required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Duration (e.g. 3 Months)</label>
                          <input
                            type="text" required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Budget (e.g. $15,000)</label>
                          <input
                            type="text" required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Completion Year</label>
                          <input
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                        <div className="space-y-1.5 col-span-2">
                          <label className="font-semibold text-gray-500">Custom URL Slug (leave empty for automatic)</label>
                          <input
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Industry (En)</label>
                          <input
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Industry (Bn)</label>
                          <input
                            type="text" placeholder="ফিনটেক"
                            value={portfolioForm.industryBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, industryBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Project Status</label>
                          <select
                            value={portfolioForm.status || 'published'}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, status: e.target.value as any })}
                            className="w-full rounded border border-gray-200 bg-white px-2 py-2 text-gray-800 focus:outline-none focus:border-blue-500 font-bold"
                          >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-2">
                        <div className="space-y-1.5 col-span-2">
                          <label className="font-semibold text-gray-500">Featured Image URL</label>
                          <input
                            type="text" required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Sort Order</label>
                          <input
                            type="number"
                        ...
                        <div className="flex items-center space-x-2 h-full pt-6">
                          ...
                          />
                          <label htmlFor="portfolio-form-featured" className="font-bold text-gray-700">Featured Project</label>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 p-4 rounded-xl border border-gray-100 bg-white">
                      <h4 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px] mb-2 border-b border-gray-50 pb-1.5">2. Deep Case Study Content</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Short Summary (English)</label>
                          <textarea
                            rows={2} required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Short Summary (Bengali)</label>
                          <textarea
                            rows={2} required
                            value={portfolioForm.descriptionBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, descriptionBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">The Challenge (English)</label>
                          <textarea
                            rows={3} required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">The Challenge (Bengali)</label>
                          <textarea
                            rows={3} required
                            value={portfolioForm.challengeBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, challengeBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Our Solution (English)</label>
                          <textarea
                            rows={3} required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Our Solution (Bengali)</label>
                          <textarea
                            rows={3} required
                            value={portfolioForm.solutionBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, solutionBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Measurable Results (English)</label>
                          <textarea
                            rows={3} required
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Measurable Results (Bengali)</label>
                          <textarea
                            rows={3} required
                            value={portfolioForm.resultBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, resultBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-50 pt-2">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Delivered Features (English, Comma Separated)</label>
                          <textarea
                            rows={2}
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Delivered Features (Bengali, Comma Separated)</label>
                          <textarea
                            rows={2}
                            placeholder="রেসপন্সিভ ড্যাশবোর্ড, বায়োমেট্রিক গেটওয়ে, ডেটা পাইপলাইন"
                            value={portfolioForm.featuresBn ? portfolioForm.featuresBn.join(', ') : ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, featuresBn: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Image Gallery (JSON List of URLs, e.g. ["url1", "url2"])</label>
                          <textarea
                            rows={2}
                            value={portfolioForm.galleryJson || '[]'}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, galleryJson: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500 font-mono"
                          />
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Before Image (URL)</label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">After Image (URL)</label>
                            <input
                              type="text"
                    ...
                    <div className="space-y-3 p-4 rounded-xl border border-gray-100 bg-white">
                      <h4 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px] mb-2 border-b border-gray-50 pb-1.5">3. Project-Specific Client Testimonial</h4>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        <div className="space-y-1.5 col-span-2">
                          <label className="font-semibold text-gray-500">Client Contact Person Photo URL</label>
                          <input
                            type="text"
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Client Role (English)</label>
                          <input
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Client Role (Bengali)</label>
                          <input
                            type="text" placeholder="e.g. সিইও"
                            value={portfolioForm.clientRoleBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, clientRoleBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Testimonial Feedback (English)</label>
                          <textarea
                            rows={2}
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Testimonial Feedback (Bengali)</label>
                          <textarea
                            rows={2}
                            value={portfolioForm.reviewBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, reviewBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Rating (1 to 5 Stars)</label>
                          <input
                    ...
                    <div className="space-y-3 p-4 rounded-xl border border-gray-100 bg-white">
                      <h4 className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px] mb-2 border-b border-gray-50 pb-1.5">4. Integration, Links & SEO</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">Technologies (Comma Separated)</label>
                          <input
                            type="text" required
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Live Project URL</label>
                            <input
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">GitHub Repository URL</label>
                            <input
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-50 pt-2">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">SEO Title (English)</label>
                          <input
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">SEO Title (Bengali)</label>
                          <input
                            type="text" placeholder="গুগল সার্চ টাইটেল"
                            value={portfolioForm.seoTitleBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, seoTitleBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">SEO Meta Description (English)</label>
                          <textarea
                        ...
                        <div className="space-y-1.5">
                          <label className="font-semibold text-gray-500">SEO Meta Description (Bengali)</label>
                          <textarea
                            rows={2} placeholder="গুগল সার্চ মেটা ডেসক্রিপশন"
                            value={portfolioForm.seoDescBn || ''}
                            onChange={(e) => setPortfolioForm({ ...portfolioForm, seoDescBn: e.target.value })}
                            className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                    ...
                    <div className="flex justify-end space-x-2 pt-4 border-t border-gray-100">
                      <button
                        ...
                        className="rounded border border-gray-200 bg-white px-4 py-2 font-bold text-gray-600 hover:bg-gray-50"
                      >
                        Cancel
                      </button>
                      <button
                        type="submit"
                        className="rounded bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 font-bold shadow-md shadow-blue-600/10"
                      >
                        Save Case Study
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {portfolios.map((item) => (
                      <div key={item.id} className="rounded-xl border border-gray-100 p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                        <div className="flex items-center space-x-4">
                          <img src={item.image} alt="" className="h-12 w-16 object-cover rounded bg-gray-50 border border-gray-100" referrerPolicy="no-referrer" />
                          <div>
                            <span className="font-bold text-sm text-gray-900 block">{item.titleEn}</span>
                            <span className="text-gray-400 font-mono text-[10px] block mt-0.5">{item.category} • Client: {item.client} • Budget: {item.budget}</span>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          <button
                            onClick={() => handleEditPortfolioTrigger(item)}
                            className="rounded bg-gray-50 hover:bg-gray-100 px-3 py-1.5 font-semibold text-gray-700 border border-gray-100 transition"
                          >
                            Edit
                          </button>
                          <button
                            onClick={() => handleDeletePortfolio(item.id)}
                            className="rounded bg-red-50 hover:bg-red-100 px-3 py-1.5 font-semibold text-red-600 border border-red-100 transition"
                          >
                            Delete
                          </button>
                        </div>
                    ...
                    {portfolios.length === 0 && (
                      <div className="text-center py-10 text-gray-400 italic">No case studies loaded. Add one to expand agency portfolio!</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <div id="panel-pricing-desk" className="space-y-6 text-xs">
                ...
                <div className="flex flex-wrap items-center gap-2 border-b border-gray-100 pb-3">
                  {[
                    ...
                    { id: 'quotes', label: currentLang === 'en' ? 'Inbound Quotes' : 'প্রাপ্ত কোটেশন', count: pricingQuotes.filter(q => q.status === 'pending').length, badgeColor: 'bg-amber-100 text-amber-800' },
                    ...
                    >
                      ...
                      {subTab.count !== undefined && (
                        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold ${subTab.badgeColor || 'bg-gray-100 text-gray-600'}`}>
                          {subTab.count}
                        </span>
                      )}
                    </button>
                ...
                {pricingSubTab === 'packages' && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      {!isCreatingPricingPackage && (
                        <button
                          ...
                          className="flex items-center space-x-1 rounded-md bg-blue-600 hover:bg-blue-700 text-white px-2.5 py-1.5 font-bold transition"
                        >
                          <PlusIcon className="h-3.5 w-3.5" />
                    ...
                    {isCreatingPricingPackage ? (
                      <form onSubmit={handleSavePricingPackage} className="space-y-4 bg-gray-50/50 rounded-xl p-4 md:p-5 border border-gray-100 max-h-[75vh] overflow-y-auto">
                        <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                          <span className="font-extrabold text-blue-600 uppercase tracking-wider text-[10px]">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Package Name (English)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Package Name (Bengali)</label>
                            <input
                              type="text" required
                              value={pricingPackageForm.nameBn || ''}
                              onChange={(e) => setPricingPackageForm({ ...pricingPackageForm, nameBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                              placeholder="যেমন: স্টার্টআপ লঞ্চপ্যাড"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Price (Monthly USD/BDT)</label>
                            <input
                              ...
                              className="w-full rounded border border-gray-200 bg-white px-2 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Price (Yearly USD/BDT)</label>
                            <input
                              type="number" required
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Package Category</label>
                            <select
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Description (Bengali)</label>
                            <textarea
                              rows={2} required
                              value={pricingPackageForm.descriptionBn || ''}
                              onChange={(e) => setPricingPackageForm({ ...pricingPackageForm, descriptionBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                              placeholder="প্যাকেজের সংক্ষিপ্ত বিবরণ বাংলায় লিখুন"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Included Features (English - Comma Separated)</label>
                            <textarea
                              rows={3} required
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Included Features (Bengali - Comma Separated)</label>
                            <textarea
                              rows={3} required
                              value={pricingPackageForm.featuresBn ? pricingPackageForm.featuresBn.join(', ') : ''}
                              onChange={(e) => setPricingPackageForm({ ...pricingPackageForm, featuresBn: e.target.value.split(',').map(f => f.trim()).filter(Boolean) })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                              placeholder="ডাইনামিক ড্যাশবোর্ড, ৩টি ফিগমা প্রোটোটাইপ"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">NOT Included Features (English - Comma Separated)</label>
                            <textarea
                              rows={2}
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">NOT Included Features (Bengali - Comma Separated)</label>
                            <textarea
                              rows={2}
                              value={pricingPackageForm.notIncludedBn ? pricingPackageForm.notIncludedBn.join(', ') : ''}
                              onChange={(e) => setPricingPackageForm({ ...pricingPackageForm, notIncludedBn: e.target.value.split(',').map(f => f.trim()).filter(Boolean) })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                              placeholder="কাস্টম সিএমএস, নেটিভ অ্যাপস ইন্টিগ্রেশন"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">CTA Text (English)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">CTA Text (Bengali)</label>
                            <input
                              type="text" required
                              value={pricingPackageForm.ctaBn || 'শুরু করুন'}
                              onChange={(e) => setPricingPackageForm({ ...pricingPackageForm, ctaBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Promo Badge (English)</label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Promo Badge (Bengali)</label>
                            <input
                              type="text"
                              placeholder="e.g. জনপ্রিয়"
                              value={pricingPackageForm.badgeBn || ''}
                              onChange={(e) => setPricingPackageForm({ ...pricingPackageForm, badgeBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                          <div className="flex items-center space-x-2">
                            ...
                            />
                            <label htmlFor="pkg-popular" className="font-bold text-gray-600">Mark as Popular (Visual Highlight)</label>
                          </div>
                          <div className="flex items-center space-x-2">
                            <input
                              ...
                              className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                            />
                            <label htmlFor="pkg-enabled" className="font-bold text-gray-600">Publish/Enable Plan</label>
                          </div>
                          <div className="space-y-1.5">
                            <input
                              type="text"
                              ...
                              className="w-full rounded border border-gray-200 bg-white px-2 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="flex justify-end space-x-2 pt-3 border-t border-gray-100">
                          <button
                            ...
                            className="rounded border border-gray-200 bg-white px-3.5 py-1.5 font-bold text-gray-600 hover:bg-gray-50"
                          >
                            Cancel
                          </button>
                          <button
                            type="submit"
                            className="rounded bg-blue-600 hover:bg-blue-700 text-white px-4.5 py-1.5 font-bold"
                          >
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {pricingPackages.map((pkg) => (
                          <div key={pkg.id} className="rounded-xl border border-gray-100 p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                            <div>
                              <div className="flex items-center space-x-2">
                                <span className="font-bold text-sm text-gray-900">{pkg.nameEn}</span>
                                <span className="text-[9px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded font-extrabold uppercase">{pkg.category}</span>
                                {pkg.popular && <span className="text-[9px] bg-amber-50 text-amber-600 px-1.5 py-0.5 rounded font-bold">Featured</span>}
                                {!pkg.enabled && <span className="text-[9px] bg-red-50 text-red-600 px-1.5 py-0.5 rounded font-bold">Disabled</span>}
                              </div>
                              <span className="text-gray-400 font-mono text-[10px] block mt-1">
                                Monthly: <span className="text-blue-600 font-bold">${pkg.priceMonthly}</span> • Yearly: <span className="text-green-600 font-bold">${pkg.priceYearly}</span> • Deliverables count: {pkg.featuresEn.length}
                              </span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button
                        ...
                        {pricingPackages.length === 0 && (
                          <div className="text-center py-8 text-gray-400 italic">No pricing packages configured. Add one to start scaling sales.</div>
                        )}
                      </div>
                    )}
                  </div>
                ...
                {pricingSubTab === 'addons' && (
                  <div className="space-y-4">
                    ...
                    {isCreatingPricingAddon ? (
                      <form onSubmit={handleSavePricingAddon} className="space-y-4 bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Add-on Name (English)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Add-on Name (Bengali)</label>
                            <input
                              type="text" required
                              value={pricingAddonForm.nameBn || ''}
                              onChange={(e) => setPricingAddonForm({ ...pricingAddonForm, nameBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                              placeholder="যেমন: এসইও লঞ্চ ক্যাম্পেইন"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Add-on Price Label (e.g. $499 or $100/hr)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Service Category Tag</label>
                            <input
                              type="text" required
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <textarea
                              rows={2} required
                              value={pricingAddonForm.descriptionBn || ''}
                              onChange={(e) => setPricingAddonForm({ ...pricingAddonForm, descriptionBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                              placeholder="অ্যাড-অন বিবরণ বাংলায় লিখুন"
                            />
                          </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 pt-1">
                          <input
                            ...
                            className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                          />
                          <label htmlFor="addon-enabled" className="font-bold text-gray-600">Add-on Active / Enabled for Estimator Calculator</label>
                        </div>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {pricingAddons.map((addon) => (
                          <div key={addon.id} className="rounded-xl border border-gray-100 p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                            <div>
                              <div className="flex items-center space-x-2">
                                <span className="font-bold text-sm text-gray-900">{addon.nameEn}</span>
                                <span className="text-[9px] bg-purple-50 text-purple-600 px-2 py-0.5 rounded font-extrabold uppercase">{addon.category}</span>
                                {!addon.enabled && <span className="text-[9px] bg-red-50 text-red-600 px-1.5 py-0.5 rounded font-bold">Disabled</span>}
                              </div>
                              <span className="text-gray-400 font-mono text-[10px] block mt-1">
                                Cost upgrade: <span className="text-blue-600 font-bold">{addon.price}</span> — {addon.descriptionEn}
                              </span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button
                        ...
                        {pricingAddons.length === 0 && (
                          <div className="text-center py-8 text-gray-400 italic">No custom add-on options configured yet.</div>
                        )}
                      </div>
                    )}
                  </div>
                ...
                {pricingSubTab === 'comparisons' && (
                  <div className="space-y-4">
                    ...
                    {isCreatingPricingComparison ? (
                      <form onSubmit={handleSavePricingComparison} className="space-y-4 bg-gray-50/50 rounded-xl p-4 border border-gray-100 max-h-[75vh] overflow-y-auto">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Group Category (English)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Group Category (Bengali)</label>
                            <input
                              type="text" required
                              value={pricingComparisonForm.categoryBn || ''}
                              onChange={(e) => setPricingComparisonForm({ ...pricingComparisonForm, categoryBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                              placeholder="যেমন: মূল বৈশিষ্ট্য, গ্রাহক সহায়তা"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Feature Name (English)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Feature Name (Bengali)</label>
                            <input
                              type="text" required
                              value={pricingComparisonForm.featureBn || ''}
                              onChange={(e) => setPricingComparisonForm({ ...pricingComparisonForm, featureBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                              placeholder="যেমন: স্ল্যাক সরাসরি সহায়তা"
                            />
                          </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 bg-white p-3 rounded-xl border border-gray-100">
                          <div className="space-y-2 border-r border-gray-50 pr-2">
                            <h5 className="font-extrabold text-blue-600 text-[10px]">1. Starter Level Specs</h5>
                            <input
                              ...
                              className="w-full rounded border border-gray-200 bg-gray-50 px-2 py-1.5 text-gray-800 focus:outline-none"
                            />
                            <input
                              type="text" required placeholder="Starter Bn (e.g. শুধুমাত্র ইমেইল)"
                              value={pricingComparisonForm.starterBn || ''}
                              onChange={(e) => setPricingComparisonForm({ ...pricingComparisonForm, starterBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-gray-50 px-2 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>

                          <div className="space-y-2 border-r border-gray-50 pr-2">
                            <h5 className="font-extrabold text-blue-600 text-[10px]">2. Business Level Specs</h5>
                            <input
                            ...
                            />
                            <input
                              type="text" required placeholder="Business Bn (e.g. পরদিন উত্তর)"
                              value={pricingComparisonForm.businessBn || ''}
                              onChange={(e) => setPricingComparisonForm({ ...pricingComparisonForm, businessBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-gray-50 px-2 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>

                          <div className="space-y-2">
                            <h5 className="font-extrabold text-blue-600 text-[10px]">3. Enterprise Level Specs</h5>
                            <input
                            ...
                            />
                            <input
                              type="text" required placeholder="Enterprise Bn (e.g. সার্বক্ষণিক ইনস্ট্যান্ট)"
                              value={pricingComparisonForm.enterpriseBn || ''}
                              onChange={(e) => setPricingComparisonForm({ ...pricingComparisonForm, enterpriseBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-gray-50 px-2 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="font-semibold text-gray-500">Global Sort Order</label>
                            <input
                              type="number"
                          ...
                          <div className="pt-6 text-gray-400 text-[10px]">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {pricingComparisons.map((comp) => (
                          <div key={comp.id} className="rounded-xl border border-gray-100 p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                            <div>
                              <div className="flex items-center space-x-2">
                                <span className="font-bold text-sm text-gray-900">{comp.featureEn}</span>
                                <span className="text-[9px] bg-slate-100 text-slate-700 px-2 py-0.5 rounded font-extrabold uppercase">{comp.categoryEn}</span>
                              </div>
                              <span className="text-gray-400 font-mono text-[10px] block mt-1">
                                Starter: <span className="text-gray-600 font-semibold">{comp.starterEn}</span> • Business: <span className="text-gray-600 font-semibold">{comp.businessEn}</span> • Enterprise: <span className="text-gray-600 font-semibold">{comp.enterpriseEn}</span>
                              </span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button
                        ...
                        {pricingComparisons.length === 0 && (
                          <div className="text-center py-8 text-gray-400 italic">No feature comparison records configured yet.</div>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {pricingQuotes.map((quote) => (
                        <div key={quote.id} className="rounded-xl border border-gray-100 p-5 shadow-sm bg-white hover:border-blue-500 transition space-y-4">
                          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-50 pb-2.5">
                            <div>
                              <span className="text-sm font-bold text-gray-900 block">{quote.name}</span>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <select
                                ...
                                className="text-[10px] bg-gray-50 border border-gray-200 rounded px-2 py-1 font-bold"
                              >
                              ...
                              <button
                                ...
                                className="rounded bg-red-50 hover:bg-red-100 text-red-600 p-1 border border-red-100 transition"
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[10px]">
                            <div className="space-y-0.5">
                              <span className="text-gray-400 font-bold block">CLIENT EMAIL</span>
                              <a href={`mailto:${quote.email}`} className="text-blue-600 hover:underline font-semibold block">{quote.email}</a>
                            </div>
                            <div className="space-y-0.5">
                              <span className="text-gray-400 font-bold block">CLIENT PHONE</span>
                              <a href={`tel:${quote.phone}`} className="text-gray-800 font-mono font-semibold block">{quote.phone}</a>
                            </div>
                            <div className="space-y-0.5">
                              <span className="text-gray-400 font-bold block">SERVICE REQUESTED</span>
                              <span className="text-gray-800 font-bold block">{quote.service}</span>
                            </div>
                            <div className="space-y-0.5">
                              <span className="text-gray-400 font-bold block">BUDGET & TIMELINE</span>
                              <span className="text-amber-600 font-bold block">{quote.budget} ({quote.timeline})</span>
                            </div>
                          </div>

                          <div className="bg-gray-50/50 rounded-lg p-3 text-gray-700 leading-relaxed font-sans text-xs border border-gray-100">
                            <span className="font-extrabold text-blue-600 text-[10px] block mb-1">PROJECT DETAILS / CHALLENGE</span>
                            {quote.description}
                          </div>

                          {quote.attachmentName && quote.attachmentData && (
                            <div className="flex items-center space-x-2 bg-emerald-50/30 border border-emerald-100 rounded-lg p-2.5 max-w-sm">
                              <ImageIcon className="h-4 w-4 text-emerald-600" />
                              <div className="text-[10px] overflow-hidden truncate">
                                <span className="font-bold text-gray-700 block truncate">{quote.attachmentName}</span>
                                <a 
                      ...
                      {pricingQuotes.length === 0 && (
                        <div className="text-center py-10 text-gray-400 italic">No custom quote requests logged yet. Inbound client inquiries will appear here.</div>
                      )}
                    </div>
                  </div>
                )}
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-4">
                      ...
                      {!isCreatingCurrency && (
                        <button
                          ...
                          className="flex items-center space-x-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 font-bold transition self-start md:self-auto"
                        >
                          <PlusIcon className="h-4 w-4" />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                      ...
                      <div className="lg:col-span-5 space-y-6">
                        ...
                        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-xs space-y-4">
                          <h5 className="font-bold text-gray-700 border-b border-gray-50 pb-2 flex items-center space-x-1.5">
                          ...
                          <form onSubmit={handleSaveCurrencySettings} className="space-y-4">
                            <div className="space-y-1.5">
                              <label className="font-semibold text-gray-500 block">{currentLang === 'en' ? 'Default Currency Code' : 'ডিফল্ট কারেন্সি কোড'}</label>
                              <select
                                ...
                                className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 font-bold focus:outline-none"
                              >
                                {currencies.map(c => (
                            ...
                            <div className="space-y-1.5">
                              <label className="font-semibold text-gray-500 block">{currentLang === 'en' ? 'Price Decimal Precision' : 'দশমিকের পর সংখ্যা'}</label>
                              <select
                            ...
                            <div className="flex items-center justify-between py-2 border-t border-b border-gray-50">
                              <div>
                                <span className="font-semibold text-gray-700 block">{currentLang === 'en' ? 'Use Live Exchange Rates' : 'লাইভ এক্সচেঞ্জ রেট ব্যবহার'}</span>
                            ...
                            <button
                              type="submit"
                              className="w-full rounded bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 shadow-xs transition"
                            >
                        ...
                        {isCreatingCurrency && (
                          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-xs space-y-4">
                            <h5 className="font-bold text-gray-700 border-b border-gray-50 pb-2 flex items-center justify-between">
                            ...
                            <form onSubmit={handleSaveCurrency} className="space-y-3">
                              <div className="space-y-1">
                                <label className="font-semibold text-gray-500 block">{currentLang === 'en' ? 'Currency Name' : 'কারেন্সির নাম'}</label>
                                <input
                                  type="text" required placeholder="e.g. US Dollar"
                                  value={currencyForm.name || ''}
                                  onChange={(e) => setCurrencyForm({ ...currencyForm, name: e.target.value })}
                                  className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                                />
                              </div>

                              <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1">
                                  <label className="font-semibold text-gray-500 block">{currentLang === 'en' ? 'Currency Code' : 'কারেন্সি কোড'}</label>
                                  <input
                                    type="text" required placeholder="e.g. USD" maxLength={3}
                                    value={currencyForm.code || ''}
                                    onChange={(e) => setCurrencyForm({ ...currencyForm, code: e.target.value.toUpperCase() })}
                                    className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 uppercase focus:outline-none font-bold"
                                  />
                                </div>
                                <div className="space-y-1">
                                  <label className="font-semibold text-gray-500 block">{currentLang === 'en' ? 'Currency Symbol' : 'কারেন্সি প্রতীক'}</label>
                                  <input
                                    type="text" required placeholder="e.g. $"
                                    value={currencyForm.symbol || ''}
                                    onChange={(e) => setCurrencyForm({ ...currencyForm, symbol: e.target.value })}
                                    className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none font-mono"
                                  />
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1">
                                  <label className="font-semibold text-gray-500 block">{currentLang === 'en' ? 'Flag Emoji' : 'পতাকা ইমোজি'}</label>
                                  <input
                                    type="text" placeholder="e.g. 🇺🇸"
                                    value={currencyForm.flag || ''}
                                    onChange={(e) => setCurrencyForm({ ...currencyForm, flag: e.target.value })}
                                    className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 text-center text-lg focus:outline-none"
                                  />
                                </div>
                                <div className="space-y-1">
                                  <label className="font-semibold text-gray-500 block">{currentLang === 'en' ? 'USD Exchange Rate' : 'USD এক্সচেঞ্জ রেট'}</label>
                                  <input
                                    ...
                                    className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none font-mono font-semibold"
                                  />
                                </div>
                              </div>
                              ...
                              <div className="space-y-1.5 pt-1.5">
                                <div className="flex items-center space-x-2">
                                  ...
                                  />
                                  <label htmlFor="curr-enabled" className="font-semibold text-gray-700">{currentLang === 'en' ? 'Enable Currency' : 'সক্রিয় করুন'}</label>
                                </div>
                                ...
                                <div className="flex items-center space-x-2">
                                  <input
                                    ...
                                    className="h-4 w-4 text-blue-600 rounded"
                                  />
                                  <label htmlFor="curr-is-default" className="font-semibold text-gray-700">{currentLang === 'en' ? 'Set as System Default' : 'সিস্টেম ডিফল্ট সেট করুন'}</label>
                                </div>
                              </div>
                      ...
                      <div className="lg:col-span-7 space-y-4">
                        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-xs space-y-4">
                          ...
                          <div className="divide-y divide-gray-100">
                                  ...
                                  <div className="flex items-center space-x-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                      <div className="flex flex-wrap items-center gap-1.5">
                                        ...
                                        <span className="text-[10px] bg-slate-100 text-slate-700 font-bold font-mono px-1.5 py-0.5 rounded">
                                          {curr.symbol}
                                        </span>
                                        {curr.isDefault && (
                                          <span className="text-[8px] bg-blue-100 text-blue-800 font-extrabold px-1.5 py-0.2 rounded-full uppercase tracking-wider">
                                      ...
                                      <span className="text-gray-400 font-semibold text-[10px] block truncate">{curr.name}</span>
                                    </div>
                                  </div>

                                  <div className="flex items-center space-x-3 shrink-0">
                                    <div className="text-right font-mono text-xs hidden sm:block">
                                      <span className="text-gray-400 text-[10px] block">{currentLang === 'en' ? 'USD Exchange Rate' : 'USD এক্সচেঞ্জ রেট'}</span>
                                      <span className="text-gray-800 font-bold">1 USD = {curr.exchangeRate} {curr.code}</span>
                                    </div>
                                    ...
                                    <div className="flex flex-col space-y-1">
                                      <button
                                        ...
                                        className="p-1 rounded hover:bg-gray-100 text-gray-400 disabled:opacity-20 disabled:hover:bg-transparent"
                                    ...
                                    <div className="flex space-x-1">
                                      <button
                                        ...
                                        className="rounded bg-gray-50 hover:bg-gray-100 px-2 py-1 font-semibold text-gray-700 border border-gray-100 transition"
                                      >
                                        Edit
                                      </button>
                                      <button
                                        ...
                                        className="rounded bg-red-50 hover:bg-red-100 px-2 py-1 font-semibold text-red-600 border border-red-100 transition disabled:opacity-30 disabled:cursor-not-allowed"
                                      >
                                        Delete
                                      </button>
                                    </div>
                            ...
                            {currencies.length === 0 && (
                              <div className="text-center py-8 text-gray-400 italic">{currentLang === 'en' ? 'No currencies configured.' : 'কোনো কারেন্সি কনফিগার করা নেই।'}</div>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            {activeSubTab === 'faqs' && (
              <div id="panel-faqs-desk" className="space-y-6 text-xs">
                ...
                {isCreatingFAQ ? (
                  <form onSubmit={handleSaveFAQ} className="space-y-4 bg-gray-50/50 rounded-xl p-4 md:p-5 border border-gray-100">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">FAQ Group Category (English)</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">FAQ Group Category (Bengali)</label>
                        <input
                          type="text" required
                          value={faqForm.categoryBn || ''}
                          onChange={(e) => setFaqForm({ ...faqForm, categoryBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Question Statement (English)</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Question Statement (Bengali)</label>
                        <input
                          type="text" required
                          value={faqForm.questionBn || ''}
                          onChange={(e) => setFaqForm({ ...faqForm, questionBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Answer Explanation (English)</label>
                        <textarea
                          rows={4} required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Answer Explanation (Bengali)</label>
                        <textarea
                          rows={4} required
                          value={faqForm.answerBn || ''}
                          onChange={(e) => setFaqForm({ ...faqForm, answerBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end space-x-2 pt-4">
                      ...
                      <button
                        type="submit"
                        className="rounded bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 font-bold"
                      >
                        Save FAQ Question
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {faqs.map((item) => (
                      <div key={item.id} className="rounded-xl border border-gray-100 p-4 shadow-sm bg-white hover:border-blue-500 transition space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-[10px] font-bold uppercase">{item.categoryEn}</span>
                          <div className="flex items-center space-x-2">
                            <button
                        ...
                        <p className="font-bold text-sm text-gray-900 font-sans">Q: {item.questionEn}</p>
                        <p className="text-gray-500 text-[11px] leading-relaxed">A: {item.answerEn}</p>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {activeSubTab === 'testimonials' && (
              <div id="panel-testimonials-desk" className="space-y-6 text-xs">
                {/* SUB TAB MENU */}
                <div className="flex flex-wrap border-b border-gray-100 gap-1 pb-px">
                  <button
                ...
                {testimonialsSubTab === 'reviews' && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      {!isCreatingTestimonial && (
                        <button
                          ...
                          className="flex items-center space-x-1 rounded bg-blue-600 hover:bg-blue-700 text-white px-2.5 py-1.5 font-bold transition"
                        >
                    ...
                    {isCreatingTestimonial ? (
                      <form onSubmit={handleSaveTestimonial} className="space-y-4 bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Name</label>
                            <input
                              type="text" required
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Role (English)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Role (Bengali)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Rating (Stars 1-5)</label>
                            <input
                              type="number" min={1} max={5} required
                              value={testimonialForm.rating || 5}
                              onChange={(e) => setTestimonialForm({ ...testimonialForm, rating: parseInt(e.target.value) || 5 })}
                              className="w-full rounded border border-gray-200 bg-white px-2 py-1.5 text-gray-800 focus:outline-none font-bold"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Avatar Image URL</label>
                            <input
                              type="text" required
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                              <label className="font-bold text-gray-500">Industry (EN)</label>
                              <input
                                type="text"
                            ...
                            <div className="space-y-1">
                              <label className="font-bold text-gray-500">Industry (BN)</label>
                              <input
                                type="text"
                                value={testimonialForm.industryBn || ''}
                                onChange={(e) => setTestimonialForm({ ...testimonialForm, industryBn: e.target.value })}
                                className="w-full rounded border border-gray-200 bg-white px-2 py-1.5 text-gray-800 focus:outline-none"
                                placeholder="যেমন: স্যাস"
                              />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="flex items-center space-x-2 pt-4">
                            ...
                            />
                            <label htmlFor="isVerified" className="font-bold text-gray-700">Verified Client Account</label>
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Feedback Review (English)</label>
                            <textarea
                              rows={4} required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Feedback Review (Bengali)</label>
                            <textarea
                              rows={4} required
                              value={testimonialForm.feedbackBn || ''}
                              onChange={(e) => setTestimonialForm({ ...testimonialForm, feedbackBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="flex justify-end space-x-2 pt-2">
                          <button
                            ...
                            className="rounded border border-gray-200 bg-white px-3 py-1.5 font-bold text-gray-600 hover:bg-gray-50"
                          >
                            Cancel
                          </button>
                          <button
                            type="submit"
                            className="rounded bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 font-bold"
                          >
                            Save Review
                          </button>
                        </div>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {testimonials.map((item) => (
                          <div key={item.id} className="rounded-xl border border-gray-100 p-4 flex items-start gap-4 bg-white hover:border-blue-500 hover:shadow-sm transition">
                            <img src={item.avatar} alt="" className="h-10 w-10 rounded-full object-cover border border-gray-100 bg-gray-50" referrerPolicy="no-referrer" />
                            <div className="flex-1 space-y-1">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-2">
                                  <span className="font-bold text-xs text-gray-900">{item.name}</span>
                                  <span className="text-gray-400 text-[10px]">• {item.company}</span>
                                  {item.isVerified && (
                                    <span className="bg-blue-50 text-blue-700 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">Verified</span>
                                  )}
                                  {item.status === 'featured' && (
                                    <span className="bg-amber-50 text-amber-800 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider">Featured</span>
                                  )}
                                </div>
                                <div className="flex items-center space-x-2">
                                  <button
                              ...
                              <span className="block text-[10px] text-gray-500">{item.roleEn} ({item.industryEn || 'General'})</span>
                              <p className="text-gray-600 leading-relaxed italic text-[11px] bg-gray-50/50 p-2 rounded mt-1 border border-gray-50">"{item.feedbackEn}"</p>
                              <div className="flex items-center space-x-0.5 text-amber-500 font-semibold pt-1">
                                {'★'.repeat(item.rating)}
                              </div>
                            </div>
                          </div>
                ...
                {testimonialsSubTab === 'videos' && (
                  <div className="space-y-4">
                    ...
                    {isCreatingVideo ? (
                      <form onSubmit={handleSaveTestimonialVideo} className="space-y-4 bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Name</label>
                            <input
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Video Title (English)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Video Title (Bengali)</label>
                            <input
                              type="text" required
                              value={videoForm.titleBn || ''}
                              onChange={(e) => setVideoForm({ ...videoForm, titleBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <div className="space-y-1 col-span-2">
                            <label className="font-bold text-gray-500">YouTube / Vimeo Embed URL</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Rating (Stars)</label>
                            <input
                              type="number" min={1} max={5} required
                              value={videoForm.rating || 5}
                              onChange={(e) => setVideoForm({ ...videoForm, rating: parseInt(e.target.value) || 5 })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none font-bold"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Video Thumbnail URL</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Avatar Photo URL</label>
                            <input
                              type="text" required
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Short Summary (EN)</label>
                            <textarea
                              rows={2} required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Short Summary (BN)</label>
                            <textarea
                              rows={2} required
                              value={videoForm.shortDescriptionBn || ''}
                              onChange={(e) => setVideoForm({ ...videoForm, shortDescriptionBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Display Order</label>
                            <input
                              type="number" required
                          ...
                          <div className="flex items-center space-x-2 pt-4">
                            <input
                              ...
                              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 h-4 w-4"
                            />
                            <label htmlFor="videoFeatured" className="font-bold text-gray-700">Featured Highlight</label>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {testimonialVideos.map((item) => (
                          <div key={item.id} className="rounded-xl border border-gray-100 p-3 bg-white flex flex-col justify-between hover:border-blue-500 transition">
                            <div className="relative rounded-lg overflow-hidden border border-gray-100 mb-2 aspect-video">
                              <img src={item.thumbnailUrl} alt="" className="w-full h-full object-cover" />
                              <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                                <span className="bg-white/90 text-blue-600 hover:bg-white text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full shadow-lg">PLAY REEL</span>
                              </div>
                            </div>
                            <div className="space-y-1 flex-1">
                              <div className="flex items-center justify-between">
                                <span className="font-bold text-xs text-gray-900">{item.clientName} • <span className="text-gray-400">{item.company}</span></span>
                                <div className="flex items-center space-x-2">
                                  <button
                              ...
                              <h5 className="font-bold text-gray-800 text-xs">{item.titleEn}</h5>
                              <p className="text-gray-500 text-[10px] leading-relaxed line-clamp-2">"{item.shortDescriptionEn}"</p>
                              <div className="flex items-center space-x-0.5 text-amber-500 pt-1">
                                {'★'.repeat(item.rating)}
                              </div>
                            </div>
                          </div>
                ...
                {testimonialsSubTab === 'stories' && (
                  <div className="space-y-4">
                    ...
                    {isCreatingSuccessStory ? (
                      <form onSubmit={handleSaveSuccessStory} className="space-y-4 bg-gray-50/50 rounded-xl p-4 border border-gray-100 max-h-[70vh] overflow-y-auto">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Name</label>
                            <input
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <input
                              type="text" required
                              value={successStoryForm.industryBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, industryBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Service Engaged (EN)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Service Engaged (BN)</label>
                            <input
                              type="text" required
                              value={successStoryForm.serviceBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, serviceBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Background (EN)</label>
                            <textarea
                              rows={2} required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Background (BN)</label>
                            <textarea
                              rows={2} required
                              value={successStoryForm.backgroundBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, backgroundBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Challenge (EN)</label>
                            <textarea
                              rows={3} required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Challenge (BN)</label>
                            <textarea
                              rows={3} required
                              value={successStoryForm.challengeBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, challengeBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Solution (EN)</label>
                            <textarea
                              rows={3} required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Solution (BN)</label>
                            <textarea
                              rows={3} required
                              value={successStoryForm.solutionBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, solutionBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Results Description (EN)</label>
                            <textarea
                              rows={2} required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Results Description (BN)</label>
                            <textarea
                              rows={2} required
                              value={successStoryForm.resultsBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, resultsBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                              placeholder="যেমন: ১৫০% ট্রাফিক বৃদ্ধি, ৪০% বাউন্স রেট হ্রাস"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Project Timeline (EN)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Project Timeline (BN)</label>
                            <input
                              type="text" required
                              value={successStoryForm.timelineBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, timelineBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2 py-1.5 text-gray-800 focus:outline-none"
                              placeholder="যেমন: ৩ মাস"
                            />
                          </div>
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Technologies (Comma Separated)</label>
                            <input
                              type="text"
                              ...
                              className="w-full rounded border border-gray-200 bg-white px-2 py-1.5 text-gray-800 focus:outline-none font-semibold text-blue-600"
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Before Image URL (Old Site)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">After Image URL (New Site)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Rep Photo</label>
                            <input
                              type="text" required
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Quote/Testimonial (EN)</label>
                            <textarea
                              rows={2} required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Quote/Testimonial (BN)</label>
                            <textarea
                              rows={2} required
                              value={successStoryForm.clientQuoteBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, clientQuoteBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Design Role (EN)</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Client Design Role (BN)</label>
                            <input
                              type="text" required
                              value={successStoryForm.clientRoleBn || ''}
                              onChange={(e) => setSuccessStoryForm({ ...successStoryForm, clientRoleBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none"
                              placeholder="যেমন: সিটিও ও কো-ফাউন্ডার"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="flex items-center space-x-2 pt-4">
                            <input
                              ...
                              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 h-4 w-4"
                            />
                            <label htmlFor="storyFeatured" className="font-bold text-gray-700">Featured Case Study</label>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {successStories.map((item) => (
                          <div key={item.id} className="rounded-xl border border-gray-100 p-4 bg-white flex items-center justify-between hover:border-blue-500 transition">
                            <div className="flex items-center space-x-4">
                              <img src={item.afterImage} alt="" className="h-14 w-20 rounded-lg object-cover border border-gray-100 bg-gray-50" referrerPolicy="no-referrer" />
                              <div>
                                <h5 className="font-bold text-gray-900 text-sm">{item.clientName} ({item.companyName})</h5>
                                <span className="text-[10px] bg-blue-50 text-blue-700 rounded px-1.5 py-0.5 font-bold uppercase tracking-wider">{item.serviceEn}</span>
                                <p className="text-gray-500 text-[10px] mt-1">Results: <span className="font-semibold text-green-600">{item.resultsEn}</span></p>
                              </div>
                            </div>
                            <div className="flex items-center space-x-2">
                              <button
                ...
                {testimonialsSubTab === 'logos' && (
                  <div className="space-y-4">
                    ...
                    {isCreatingClientLogo ? (
                      <form onSubmit={handleSaveClientLogo} className="space-y-4 bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Partner / Brand Name</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-500">Logo SVG / PNG URL</label>
                            <input
                              type="text" required
                              value={clientLogoForm.logoUrl || ''}
                              onChange={(e) => setClientLogoForm({ ...clientLogoForm, logoUrl: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-1.5 text-gray-800 focus:outline-none font-mono text-[10px]"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="flex items-center space-x-2 pt-4">
                            <input
                              ...
                              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 h-4 w-4"
                            />
                            <label htmlFor="logoFeatured" className="font-bold text-gray-700">Display in Slider</label>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {clientLogos.map((item) => (
                          <div key={item.id} className="rounded-xl border border-gray-100 p-3 bg-white flex flex-col items-center justify-between hover:border-blue-500 transition">
                            <div className="h-10 flex items-center justify-center p-2 mb-2 w-full bg-gray-50 rounded-lg">
                              {item.logoUrl ? (
                                <img src={item.logoUrl} alt={item.name} className="max-h-6 max-w-[80%] object-contain opacity-70" referrerPolicy="no-referrer" />
                              ) : (
                                <span className="font-bold text-[10px] text-gray-400">{item.name}</span>
                              )}
                            </div>
                            <div className="flex items-center justify-between w-full border-t border-gray-50 pt-2 mt-1">
                              <span className="font-bold text-[10px] text-gray-700 truncate max-w-[60%]">{item.name}</span>
                ...
                {testimonialsSubTab === 'stats_settings' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <form onSubmit={handleSaveTestimonialStatistics} className="space-y-3 bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                        <div className="space-y-1">
                          <label className="font-bold text-gray-500">Total Clients Served</label>
                          <input
                            type="number" required
                        ...
                        <div className="space-y-1">
                          <label className="font-bold text-gray-500">Satisfaction Rate (%)</label>
                          <input
                        ...
                        <div className="space-y-1">
                          <label className="font-bold text-gray-500">Average Rating (Stars 1-5)</label>
                          <input
                        ...
                        <div className="space-y-1">
                          <label className="font-bold text-gray-500">Success Case Studies Count</label>
                          <input
                            type="number" required
                        ...
                        <button
                          type="submit"
                          className="w-full rounded bg-blue-600 hover:bg-blue-700 text-white py-1.5 font-bold transition"
                        >
                    ...
                    <div className="space-y-4">
                      ...
                      <form onSubmit={handleSaveReviewSettings} className="space-y-3 bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                        <div className="flex items-center justify-between py-1 border-b border-gray-50">
                          <div className="flex flex-col">
                            <span className="font-bold text-gray-700">Require Approval</span>
                            <span className="text-[10px] text-gray-400">Newly submitted reviews must be manually approved</span>
                          </div>
                          <input
                            type="checkbox"
                        ...
                        <div className="flex items-center justify-between py-1 border-b border-gray-50">
                          <div className="flex flex-col">
                            <span className="font-bold text-gray-700">Enable Star Ratings</span>
                            <span className="text-[10px] text-gray-400">Allow users to provide 1-5 rating sliders</span>
                          </div>
                          <input
                            type="checkbox"
                        ...
                        <div className="flex items-center justify-between py-1 border-b border-gray-50">
                          <div className="flex flex-col">
                            <span className="font-bold text-gray-700">Allow Video Submissions</span>
                            <span className="text-[10px] text-gray-400">Enable video URL suggestions in client forms</span>
                          </div>
                          <input
                            type="checkbox"
                        ...
                        <div className="space-y-1">
                          <label className="font-bold text-gray-500">Moderator Notification Email</label>
                          <input
                            type="email" required
                        ...
                        <div className="space-y-1">
                          <label className="font-bold text-gray-500">Reviews Per Page</label>
                          <input
                            type="number" required
            ...
            {activeSubTab === 'team' && (
              <div id="panel-team-desk" className="space-y-6 text-xs">
                ...
                {isCreatingTeamMember ? (
                  <form onSubmit={handleSaveTeamMember} className="space-y-4 bg-gray-50/50 rounded-xl p-4 md:p-5 border border-gray-100">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Avatar Photo URL</label>
                        <input
                          type="text" required
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Corporate Role (English)</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Corporate Role (Bengali)</label>
                        <input
                          type="text" required
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      <div className="space-y-1.5 col-span-2">
                        <label className="font-semibold text-gray-500">LinkedIn Profile Link</label>
                        <input
                          type="text"
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Department (English)</label>
                        <select
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Department (Bengali)</label>
                        <input
                          type="text"
                          value={teamMemberForm.departmentBn || ''}
                          onChange={(e) => setTeamMemberForm({ ...teamMemberForm, departmentBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Short Bio (English)</label>
                        <textarea
                          rows={3}
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Short Bio (Bengali)</label>
                        <textarea
                          rows={3}
                          value={teamMemberForm.bioBn || ''}
                          onChange={(e) => setTeamMemberForm({ ...teamMemberForm, bioBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {teamMembers.map((member) => (
                      <div key={member.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                        <div className="flex items-center space-x-4">
                          <img src={member.avatar} alt="" className="h-10 w-10 object-cover rounded-full bg-gray-50 border border-gray-50" referrerPolicy="no-referrer" />
                          <div>
                            <span className="font-bold text-sm text-gray-900 block">{member.name}</span>
                            <span className="text-gray-400 font-mono text-[10px] block mt-0.5">{member.roleEn} • {member.departmentEn}</span>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          <button
            ...
            {activeSubTab === 'media' && (
              <div id="panel-media-desk" className="space-y-6 text-xs">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  ...
                  <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Dynamic CDN references</span>
                </div>

                {/* Add new media reference form */}
                <form onSubmit={handleAddMedia} className="bg-gray-50/50 p-4 rounded-xl border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                  <div className="space-y-1.5">
                    <label className="font-semibold text-gray-500">Asset Title</label>
                    <input
                      type="text" required
                  ...
                  <div className="space-y-1.5 col-span-2">
                    <label className="font-semibold text-gray-500">Direct CDN / Unsplash URL</label>
                    <input
                      type="text" required
                  ...
                  <button
                    type="submit"
                    className="rounded-lg bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 font-bold transition h-8.5"
                  >
                    Index Asset
                  </button>
                </form>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2 bg-gray-50 border border-gray-100 px-2.5 py-1 rounded-lg w-64 text-[11px]">
                      ...
                      {['all', 'portfolio', 'about', 'services'].map((cat) => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase transition ${
                            mediaCategory === cat ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'
                          }`}
                        >
                          {cat}
                        </button>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {mediaItems
                      ...
                      .map((item) => (
                        <div key={item.id} className="group relative rounded-xl border border-gray-100 overflow-hidden shadow-sm bg-white flex flex-col justify-between">
                          <img src={item.url} alt="" className="h-28 w-full object-cover bg-gray-50" referrerPolicy="no-referrer" />
                          <div className="p-2.5 space-y-1 border-t border-gray-50">
                            ...
                            <span className="text-[9px] text-gray-400 font-mono block truncate">{item.url}</span>
                          </div>
                          <div className="flex border-t border-gray-50 divide-x divide-gray-50 text-[10px] font-bold text-center">
                            <button
                              ...
                              className="w-1/2 py-1.5 text-blue-600 hover:bg-blue-50/50 transition cursor-pointer"
                            >
                              Copy Link
                            </button>
                            <button
                              ...
                              className="w-1/2 py-1.5 text-red-500 hover:bg-red-50/50 transition cursor-pointer"
                            >
                              Delete
                            </button>
                          </div>
            ...
            {activeSubTab === 'users' && (
              <div id="panel-users-desk" className="space-y-6 text-xs">
                ...
                {isCreatingUser ? (
                  <form onSubmit={handleSaveUser} className="space-y-4 bg-gray-50/50 rounded-xl p-4 md:p-5 border border-gray-100">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Username Identifier</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Corporate Email</label>
                        <input
                          type="email" required
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Role Permissions</label>
                        <select
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Account Status</label>
                        <select
                      ...
                      <div className="space-y-1.5">
                        <label className="font-semibold text-gray-500">Access Key Passcode</label>
                        <input
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {adminUsers.map((u) => (
                      <div key={u.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <span className="font-bold text-sm text-gray-900 block">{u.username} • <span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded text-gray-600 font-extrabold uppercase">{u.role}</span></span>
                            <span className="text-gray-400 font-mono text-[10px] block mt-0.5">{u.email} • Status: <span className={`font-bold uppercase ${u.status === 'active' ? 'text-emerald-500' : 'text-red-500'}`}>{u.status}</span></span>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          <button
              ...
              <div id="panel-whychooseus-desk" className="space-y-6">
                ...
                {whyChooseUsSubTab === 'cards' && (
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      ...
                      {!isCreatingWhyCard && !editingWhyCard && (
                        <button
                          ...
                          className="rounded bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-3 py-1.5 flex items-center gap-1 transition"
                        >
                          <PlusIcon className="h-3.5 w-3.5" />
                    ...
                    {(isCreatingWhyCard || editingWhyCard) ? (
                      <form onSubmit={handleSaveWhyCard} className="space-y-4 rounded-xl border border-blue-100 bg-blue-50/20 p-5">
                        <div className="text-xs font-bold text-blue-700 pb-2 border-b border-blue-100 flex justify-between items-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Title (English)' : 'শিরোনাম (ইংরেজী)'}</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Title (Bangla)' : 'শিরোনাম (বাংলা)'}</label>
                            <input
                              type="text" required
                              value={whyCardForm.titleBn || ''}
                              onChange={(e) => setWhyCardForm({ ...whyCardForm, titleBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>

                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Category/Spoke Key (English)' : 'ক্যাটাগরি/স্পোক কি (ইংরেজী)'}</label>
                            <input
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Category/Spoke Key (Bangla)' : 'ক্যাটাগরি/স্পোক কি (বাংলা)'}</label>
                            <input
                              type="text" placeholder="উদাঃ ডিজাইন, ডেভেলপমেন্ট, মার্কেটিং"
                              value={whyCardForm.categoryBn || ''}
                              onChange={(e) => setWhyCardForm({ ...whyCardForm, categoryBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>

                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Lucide Icon Name' : 'লুসিড আইকন নাম'}</label>
                            <select
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Badge Overlap Text (English) - Optional' : 'ওভারল্যাপ ব্যাজ টেক্সট (ইংরেজী)'}</label>
                            <input
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Badge Overlap Text (Bangla) - Optional' : 'ওভারল্যাপ ব্যাজ টেক্সট (বাংলা)'}</label>
                            <input
                              type="text" placeholder="উদাঃ গতি ১০০%"
                              value={whyCardForm.badgeTextBn || ''}
                              onChange={(e) => setWhyCardForm({ ...whyCardForm, badgeTextBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>

                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Display Order' : 'প্রদর্শনের ক্রম'}</label>
                            <input
                              type="number" required
                          ...
                          <div className="space-y-1 flex items-center pt-5">
                            ...
                            />
                            <label htmlFor="whycard-visible" className="font-bold text-gray-600 select-none">{currentLang === 'en' ? 'Card is Visible' : 'কার্ডটি দৃশ্যমান হবে'}</label>
                          </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs mt-2">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Description (English)' : 'বর্ণনা (ইংরেজী)'}</label>
                            <textarea
                              rows={3} required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Description (Bangla)' : 'বর্ণনা (বাংলা)'}</label>
                            <textarea
                              rows={3} required
                              value={whyCardForm.descriptionBn || ''}
                              onChange={(e) => setWhyCardForm({ ...whyCardForm, descriptionBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>
                        </div>
                        ...
                        <div className="flex justify-end space-x-2 pt-2 border-t border-blue-50">
                          <button
                            type="button"
                            onClick={() => { setIsCreatingWhyCard(false); setEditingWhyCard(null); }}
                            className="rounded border border-gray-200 bg-white px-4 py-2 text-xs font-bold text-gray-600 hover:bg-gray-50 transition"
                          >
                            {currentLang === 'en' ? 'Cancel' : 'বাতিল'}
                          </button>
                          <button
                            type="submit"
                            className="rounded bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 text-xs font-bold transition shadow-sm"
                          >
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {whyChooseUsCards.map((c) => (
                          <div key={c.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-[10px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded uppercase font-bold">{c.icon}</span>
                                {c.categoryEn && (
                                  <span className="font-mono text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded font-semibold">{c.categoryEn}</span>
                                )}
                                {!c.visible && (
                                  <span className="font-mono text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded font-bold">Hidden</span>
                                )}
                              </div>
                              <h5 className="font-bold text-sm text-gray-900 mt-2">
                              ...
                              <span className="text-[10px] font-mono text-gray-400 mt-1 block">Sort order: {c.displayOrder}</span>
                            </div>
                            ...
                            <div className="flex items-center space-x-2 shrink-0">
                              <button
                                ...
                                className="rounded bg-gray-50 hover:bg-gray-100 px-3 py-1.5 text-xs font-bold text-gray-700 border border-gray-100 transition"
                              >
                                {currentLang === 'en' ? 'Edit' : 'সম্পাদনা'}
                              </button>
                              <button
                                onClick={() => handleDeleteWhyCard(c.id)}
                                className="rounded bg-red-50 hover:bg-red-100 px-3 py-1.5 text-xs font-bold text-red-600 border border-red-100 transition"
                              >
                ...
                {whyChooseUsSubTab === 'stats' && (
                  <div className="space-y-6">
                    ...
                    {(isCreatingWhyStat || editingWhyStat) ? (
                      <form onSubmit={handleSaveWhyStat} className="space-y-4 rounded-xl border border-blue-100 bg-blue-50/20 p-5">
                        <div className="text-xs font-bold text-blue-700 pb-2 border-b border-blue-100 flex justify-between items-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Value (e.g. 99% or 250+)' : 'মান (উদাঃ ৯৯% বা ২৫০+)'}</label>
                            <input
                              type="text" required
                              value={whyStatForm.value || ''}
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Label (English)' : 'লেবেল (ইংরেজী)'}</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Label (Bangla)' : 'লেবেল (বাংলা)'}</label>
                            <input
                              type="text" required
                              value={whyStatForm.labelBn || ''}
                              onChange={(e) => setWhyStatForm({ ...whyStatForm, labelBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>
                          ...
                          <div className="space-y-1 flex items-center pt-2 col-span-2">
                            <input
                              ...
                              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-2 h-4 w-4"
                            />
                            <label htmlFor="whystat-visible" className="font-bold text-gray-600 select-none">{currentLang === 'en' ? 'Stat is Visible' : 'পরিসংখ্যানটি দৃশ্যমান হবে'}</label>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {whyChooseUsStats.map((s) => (
                          <div key={s.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-base font-black text-blue-600">{s.value}</span>
                                {!s.visible && (
                                  <span className="font-mono text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded font-bold">Hidden</span>
                                )}
                              </div>
                              <h5 className="font-semibold text-xs text-gray-700 mt-1">
                                {currentLang === 'en' ? s.labelEn : s.labelBn}
                              </h5>
                              <span className="text-[10px] font-mono text-gray-400 block mt-1">Sort order: {s.displayOrder}</span>
                            </div>
                ...
                {whyChooseUsSubTab === 'badges' && (
                  <div className="space-y-6">
                    ...
                    {(isCreatingWhyBadge || editingWhyBadge) ? (
                      <form onSubmit={handleSaveWhyBadge} className="space-y-4 rounded-xl border border-blue-100 bg-blue-50/20 p-5">
                        <div className="text-xs font-bold text-blue-700 pb-2 border-b border-blue-100 flex justify-between items-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                          ...
                          <div className="space-y-1">
                            ...
                            <input
                              type="text" required
                              value={whyBadgeForm.labelBn || ''}
                              onChange={(e) => setWhyBadgeForm({ ...whyBadgeForm, labelBn: e.target.value })}
                              className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                            />
                          </div>
                          ...
                          <div className="space-y-1 flex items-center pt-5">
                            <input
                              ...
                              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-2 h-4 w-4"
                            />
                            <label htmlFor="whybadge-visible" className="font-bold text-gray-600 select-none">{currentLang === 'en' ? 'Badge is Visible' : 'ব্যাজটি দৃশ্যমান হবে'}</label>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {whyChooseUsBadges.map((b) => (
                          <div key={b.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-bold text-sm text-gray-900">{currentLang === 'en' ? b.labelEn : b.labelBn}</span>
                                {!b.visible && (
                                  <span className="font-mono text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded font-bold">Hidden</span>
                                )}
                              </div>
                              <span className="text-[10px] font-mono text-gray-400 block mt-1">Sort order: {b.displayOrder}</span>
                            </div>
                ...
                {whyChooseUsSubTab === 'techs' && (
                  <div className="space-y-6">
                    ...
                    {(isCreatingWhyTech || editingWhyTech) ? (
                      <form onSubmit={handleSaveWhyTech} className="space-y-4 rounded-xl border border-blue-100 bg-blue-50/20 p-5">
                        <div className="text-xs font-bold text-blue-700 pb-2 border-b border-blue-100 flex justify-between items-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Technology Name' : 'প্রযুক্তির নাম'}</label>
                            <input
                              type="text" required
                          ...
                          <div className="space-y-1">
                            <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'SVG Logo Source / URL' : 'এসভিজি লোগো বা ছবির লিংক'}</label>
                            <input
                          ...
                          <div className="space-y-1 flex items-center pt-5">
                            <input
                              ...
                              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-2 h-4 w-4"
                            />
                            <label htmlFor="whytech-visible" className="font-bold text-gray-600 select-none">{currentLang === 'en' ? 'Tech is Visible' : 'প্রযুক্তিটি দৃশ্যমান হবে'}</label>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {whyChooseUsTechs.map((t) => (
                          <div key={t.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                            <div className="flex items-center gap-3">
                              {t.logoUrl ? (
                                <img src={t.logoUrl} alt={t.name} referrerPolicy="no-referrer" className="h-7 w-7 object-contain bg-slate-50 p-1 rounded border border-gray-100 shrink-0" />
                              ) : (
                                <div className="h-7 w-7 rounded bg-gray-100" />
                              )}
                              <div>
                                <span className="font-bold text-sm text-gray-900 block">{t.name}</span>
                                <span className="text-[9px] font-mono text-gray-400 block">Sort order: {t.displayOrder} {!t.visible && '• Hidden'}</span>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center space-x-2 shrink-0">
                              <button
                                ...
                                className="rounded bg-gray-50 hover:bg-gray-100 px-2.5 py-1.5 text-xs font-bold text-gray-700 border border-gray-100 transition"
                              >
                                {currentLang === 'en' ? 'Edit' : 'সম্পাদনা'}
                              </button>
                              <button
                                onClick={() => handleDeleteWhyTech(t.id)}
                                className="rounded bg-red-50 hover:bg-red-100 px-2.5 py-1.5 text-xs font-bold text-red-600 border border-red-100 transition"
                              >
                                {currentLang === 'en' ? 'Delete' : 'মুছে ফেলুন'}
                              </button>
                            </div>
                ...
                {whyChooseUsSubTab === 'cta' && (
                  <form onSubmit={handleSaveWhyCTA} className="space-y-4 rounded-xl border border-gray-100 bg-white p-5 shadow-sm text-xs">
                    <h4 className="text-sm font-bold text-gray-800 pb-2 border-b border-gray-100">
                      {currentLang === 'en' ? 'Call To Action (CTA) Box Text & Translations' : 'কল টু অ্যাকশন (CTA) বক্স টেক্সট ও অনুবাদসমূহ'}
                    </h4>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Tagline Over Heading (English)' : 'ট্যাগলাইন ওভার হেডিং (ইংরেজী)'}</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Tagline Over Heading (Bangla)' : 'ট্যাগলাইন ওভার হেডিং (বাংলা)'}</label>
                        <input
                          type="text" required
                          value={whyCTAForm.taglineBn || ''}
                          onChange={(e) => setWhyCTAForm({ ...whyCTAForm, taglineBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Headline (English)' : 'হেডলাইন (ইংরেজী)'}</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Headline (Bangla)' : 'হেডলাইন (বাংলা)'}</label>
                        <input
                          type="text" required
                          value={whyCTAForm.headlineBn || ''}
                          onChange={(e) => setWhyCTAForm({ ...whyCTAForm, headlineBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Primary Button Text (English)' : 'প্রধান বাটন টেক্সট (ইংরেজী)'}</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Primary Button Text (Bangla)' : 'প্রধান বাটন টেক্সট (বাংলা)'}</label>
                        <input
                          type="text" required
                          value={whyCTAForm.primaryButtonTextBn || ''}
                          onChange={(e) => setWhyCTAForm({ ...whyCTAForm, primaryButtonTextBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Secondary Button Text (English)' : 'सहयोगी বাটন টেক্সট (ইংরেজী)'}</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Secondary Button Text (Bangla)' : 'सहযোগী বাটন টেক্সট (বাংলা)'}</label>
                        <input
                          type="text" required
                          value={whyCTAForm.secondaryButtonTextBn || ''}
                          onChange={(e) => setWhyCTAForm({ ...whyCTAForm, secondaryButtonTextBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <textarea
                          rows={3} required
                          value={whyCTAForm.descriptionBn || ''}
                          onChange={(e) => setWhyCTAForm({ ...whyCTAForm, descriptionBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end pt-3 border-t border-gray-100">
                      <button
                        type="submit"
                        className="rounded bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 text-xs font-bold transition shadow-sm"
                      >
            ...
            {activeSubTab === 'techstack' && (
              <div id="panel-techstack-desk" className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className="inline-flex items-center space-x-1 rounded border border-gray-200 bg-white hover:bg-gray-50 px-3 py-1.5 text-xs font-bold text-gray-600 transition"
                    >
                      <RotateCcwIcon className="h-3.5 w-3.5" />
                    ...
                    {!isCreatingTechServiceCard && !editingTechServiceCard && (
                      <button
                        ...
                        className="inline-flex items-center space-x-1 rounded bg-blue-600 hover:bg-blue-700 px-3.5 py-1.5 text-xs font-bold text-white transition shadow-sm"
                      >
                        <PlusIcon className="h-3.5 w-3.5" />
                ...
                {isCreatingTechServiceCard || editingTechServiceCard ? (
                  <form onSubmit={handleSaveTechServiceCard} className="space-y-4 rounded-xl border border-gray-100 bg-white p-5 shadow-sm text-xs">
                    <h4 className="text-sm font-bold text-gray-800 pb-2 border-b border-gray-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Icon Class/Name' : 'আইকন নাম'}</label>
                        <select
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Category (English)' : 'ক্যাটাগরি (ইংরেজী)'}</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Category (Bangla)' : 'ক্যাটাগরি (বাংলা)'}</label>
                        <input
                          type="text" required
                          value={techServiceCardForm.categoryBn || ''}
                          onChange={(e) => setTechServiceCardForm({ ...techServiceCardForm, categoryBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <textarea
                          rows={2} required
                          value={techServiceCardForm.descriptionBn || ''}
                          onChange={(e) => setTechServiceCardForm({ ...techServiceCardForm, descriptionBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Project Count Metric (e.g. 120+)' : 'প্রজেক্ট সংখ্যা বা মেট্রিক'}</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Experience Level (English)' : 'অভিজ্ঞতা স্তর (ইংরেজী)'}</label>
                        <input
                          type="text" required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Experience Level (Bangla)' : 'অভিজ্ঞতা স্তর (বাংলা)'}</label>
                        <input
                          type="text" required
                          value={techServiceCardForm.experienceLevelBn || ''}
                          onChange={(e) => setTechServiceCardForm({ ...techServiceCardForm, experienceLevelBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Featured Badge (English) - Optional' : 'ফিচারড ব্যাজ (ইংরেজী)'}</label>
                        <input
                          type="text"
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Featured Badge (Bangla) - Optional' : 'ফিচারড ব্যাজ (বাংলা)'}</label>
                        <input
                          type="text"
                          value={techServiceCardForm.featuredBadgeBn || ''}
                          onChange={(e) => setTechServiceCardForm({ ...techServiceCardForm, featuredBadgeBn: e.target.value })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Technologies (One per line)' : 'টেকনোলজিস (লাইন প্রতি একটি)'}</label>
                        <textarea
                          rows={4} required
                          placeholder="React&#10;Next.js&#10;TailwindCSS"
                          ...
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none font-mono"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Popular Solutions (English, One per line)' : 'জনপ্রিয় সমাধান (ইংরেজী, লাইন প্রতি একটি)'}</label>
                        <textarea
                          rows={4} required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Popular Solutions (Bangla, One per line)' : 'জনপ্রিয় সমাধান (বাংলা, লাইন প্রতি একটি)'}</label>
                        <textarea
                          rows={4} required
                          placeholder="এন্টারপ্রাইজ সিআরএম&#10;স্যাস ড্যাশবোর্ড"
                          value={techServiceCardForm.popularProjectsBn ? techServiceCardForm.popularProjectsBn.join('\n') : ''}
                          onChange={(e) => setTechServiceCardForm({ ...techServiceCardForm, popularProjectsBn: e.target.value.split('\n').map(s => s.trim()).filter(Boolean) })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Core Benefits (English, One per line)' : 'মূল সুবিধা (ইংরেজী, লাইন প্রতি একটি)'}</label>
                        <textarea
                          rows={3} required
                      ...
                      <div className="space-y-1">
                        <label className="font-bold text-gray-600 block">{currentLang === 'en' ? 'Core Benefits (Bangla, One per line)' : 'মূল সুবিধা (বাংলা, লাইন প্রতি একটি)'}</label>
                        <textarea
                          rows={3} required
                          placeholder="বিদ্যুৎ গতি লোডটাইম&#10;এসইও ফ্রেন্ডলি"
                          value={techServiceCardForm.benefitsBn ? techServiceCardForm.benefitsBn.join('\n') : ''}
                          onChange={(e) => setTechServiceCardForm({ ...techServiceCardForm, benefitsBn: e.target.value.split('\n').map(s => s.trim()).filter(Boolean) })}
                          className="w-full rounded border border-gray-200 bg-white px-2.5 py-2 text-gray-800 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="space-y-1 flex items-center pt-5">
                        <input
                          ...
                          className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-2 h-4 w-4"
                        />
                        <label htmlFor="techcard-visible" className="font-bold text-gray-600 select-none">{currentLang === 'en' ? 'Card is Visible' : 'কার্ডটি দৃশ্যমান হবে'}</label>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {techServiceCards.map((card) => (
                      <div key={card.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 shadow-sm bg-white hover:border-blue-500 transition">
                        <div className="flex items-center gap-3">
