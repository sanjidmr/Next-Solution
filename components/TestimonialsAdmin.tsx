...
export default function TestimonialsAdmin({ currentLang, isOpen, onClose, onDataChanged }: TestimonialsAdminProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-md flex justify-end animate-fadeIn">
      <div className="w-full max-w-5xl bg-white min-h-screen shadow-2xl flex flex-col border-l border-slate-200">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="font-display font-bold text-lg tracking-tight flex items-center">
                Next Solution Admin Dashboard <span className="ml-2.5 text-[10px] tracking-wider uppercase font-extrabold bg-cyan-500 text-slate-950 px-2 py-0.5 rounded">CMS Console</span>
              </h2>
              <p className="text-xs text-slate-400">Fully reactive CRUD modules, database configuration, and section visibility controls</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex bg-slate-100 border-b border-slate-200 overflow-x-auto scrollbar-none">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
          ...
          {activeTab === 'reviews' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-display font-bold text-base text-slate-900">Manage Verified Customer Feed</h3>
                  <p className="text-xs text-slate-500">Edit, approve, or delete public and seeded review testimonials</p>
                </div>
                <button
                  onClick={() => setEditingReview({})}
                  className="rounded-lg bg-blue-600 hover:bg-blue-700 text-white px-3.5 py-1.5 text-xs font-bold transition flex items-center space-x-1"
                >
              ...
              {/* Review Editor Form */}
              {editingReview && (
                <form onSubmit={handleSaveReview} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    ...
                    <button type="button" onClick={() => setEditingReview(null)} className="text-slate-400 hover:text-slate-600 text-xs">Cancel</button>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Client Full Name *</label>
                      <input 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Client Role (English) *</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Client Role (Bangla)</label>
                      <input 
                        type="text" value={editingReview.roleBn || ''} 
                        onChange={e => setEditingReview({...editingReview, roleBn: e.target.value})}
                        className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs focus:border-blue-500 focus:outline-none"
                        placeholder="ভিপি অফ ডিজিটাল এক্সপেরিয়েন্স"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Flag Emoji</label>
                      <input 
                        type="text" value={editingReview.countryFlag || '🇺🇸'} 
                        onChange={e => setEditingReview({...editingReview, countryFlag: e.target.value})}
                        className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs text-center focus:border-blue-500 focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Star Rating (1-5)</label>
                      <select 
                        value={editingReview.rating || 5} 
                        onChange={e => setEditingReview({...editingReview, rating: Number(e.target.value)})}
                        className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs bg-white focus:border-blue-500 focus:outline-none"
                      >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Client Avatar Image URL</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Capability Category</label>
                      <select 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Feedback Content (English) *</label>
                    <textarea 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Feedback Content (Bangla)</label>
                    <textarea 
                      rows={3} value={editingReview.feedbackBn || ''} 
                      onChange={e => setEditingReview({...editingReview, feedbackBn: e.target.value})}
                      className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs focus:border-blue-500 focus:outline-none"
                      placeholder="তাদের ডিজাইন অত্যন্ত চমৎকার..."
                    />
                  </div>

                  <div className="flex flex-wrap gap-6 bg-slate-50 rounded-lg p-3">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <select 
                        ...
                        className="rounded border border-slate-200 px-2 py-0.5 text-xs bg-white focus:outline-none"
                      >
                  ...
                  <div className="flex justify-end space-x-2 pt-2">
                    <button 
                      ...
                      className="rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 px-4 py-2 text-xs font-bold transition"
                    >
                      Cancel
                    </button>
                    <button 
                      type="submit"
                      className="rounded-lg bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 text-xs font-bold transition shadow-sm"
                    >
                      Save Testimonial
                    </button>
                  </div>
                        ...
                        <tr key={review.id} className="hover:bg-slate-50/50 transition">
                          <td className="py-3 px-4">
                            <div className="flex items-center space-x-2.5">
                              <img src={review.avatar} alt="" className="h-8 w-8 rounded-full object-cover border border-slate-200" />
                              <div>
                                <span className="font-bold text-slate-900 flex items-center">
                                  {review.name} <span className="ml-1 text-xs">{review.countryFlag}</span>
                                </span>
                                <span className="text-[10px] text-slate-400 block">{review.roleEn}</span>
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-4 font-medium text-slate-600">
                            <span className="block font-bold text-slate-800">{review.company}</span>
                            <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded uppercase font-semibold">{review.category}</span>
                          </td>
                          ...
                          <td className="py-3 px-4 text-right space-x-1.5">
                            <button 
                              ...
                              className="p-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 transition inline-block"
                            >
                              <Edit2 className="h-3.5 w-3.5" />
                            </button>
                            <button 
                              onClick={() => handleDeleteReview(review.id)}
                              className="p-1 rounded bg-rose-50 hover:bg-rose-100 text-rose-600 transition inline-block"
                            >
          ...
          {activeTab === 'stories' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-display font-bold text-base text-slate-900">Featured Case Study Stories (Before vs After)</h3>
                  <p className="text-xs text-slate-500">Edit detailed metrics, background, solutions, and imagery deliverables</p>
                </div>
                <button
              ...
              {/* Story Form */}
              {editingStory && (
                <form onSubmit={handleSaveStory} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    <h4 className="font-bold text-sm text-slate-950">
                      {editingStory.id ? 'Edit Case Study Story' : 'Create Case Study Story'}
                    </h4>
                    <button type="button" onClick={() => setEditingStory(null)} className="text-slate-400 hover:text-slate-600 text-xs">Cancel</button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Industry (English) *</label>
                      <input 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Service Performed (English)</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Service Performed (Bangla)</label>
                      <input 
                        type="text" value={editingStory.serviceBn || ''} 
                        onChange={e => setEditingStory({...editingStory, serviceBn: e.target.value})}
                        className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs focus:border-blue-500 focus:outline-none"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Before Redesign Image URL</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">After Deliverable Image URL</label>
                      <input 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Client Name</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Client Role (EN)</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Timeline (EN)</label>
                      <input 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Challenge Summary (English) *</label>
                    <textarea 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Our Solution Summary (English) *</label>
                    <textarea 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Accomplished Results Metrics (English) *</label>
                    <textarea 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Client Quote Review (English) *</label>
                    <textarea 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {stories.map((story) => (
                  <div key={story.id} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm flex flex-col justify-between space-y-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] uppercase font-bold text-blue-600 tracking-wider bg-blue-50 px-2 py-0.5 rounded">{story.industryEn}</span>
                        <h4 className="font-display font-bold text-sm text-slate-900 mt-2">{story.companyName}</h4>
                        <p className="text-xs text-slate-500 mt-1">{story.serviceEn}</p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] border-t border-slate-100 pt-3 text-slate-500">
                      <div>
                        <strong className="text-slate-700 block">Deliverables Image:</strong>
                        <img src={story.afterImage} alt="" className="h-14 w-full object-cover rounded mt-1 border border-slate-100" />
                      </div>
                      <div>
                        <strong className="text-slate-700 block">Client Quote:</strong>
                        <p className="italic truncate mt-1">"{story.clientQuoteEn}"</p>
                        <p className="font-bold text-[9px] text-slate-400 block mt-1">{story.clientName}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {activeTab === 'videos' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-display font-bold text-base text-slate-900">Manage Video Reviews</h3>
                  <p className="text-xs text-slate-500">Add or edit video URLs, descriptions, and ratings</p>
                </div>
                <button
              ...
              {/* Video form */}
              {editingVideo && (
                <form onSubmit={handleSaveVideo} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    <h4 className="font-bold text-sm text-slate-950">
                      {editingVideo.id ? 'Edit Video Review' : 'Create Video Review'}
                    </h4>
                    <button type="button" onClick={() => setEditingVideo(null)} className="text-slate-400 hover:text-slate-600 text-xs">Cancel</button>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Video Title (English) *</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Video Title (Bangla)</label>
                      <input 
                        type="text" value={editingVideo.titleBn || ''} 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Rating</label>
                      <select 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Video YouTube Embed URL *</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Thumbnail Image URL *</label>
                      <input 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Description (English)</label>
                    <textarea 
                ...
                {videos.map((vid) => (
                  <div key={vid.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm flex flex-col justify-between">
                    <div className="relative aspect-video bg-slate-900">
                      ...
                      <div className="absolute top-3 right-3 flex space-x-1.5 z-10">
                        <button 
                          ...
                          className="p-1.5 rounded-lg bg-white/90 hover:bg-white text-slate-700 transition"
                        >
                          <Edit2 className="h-3.5 w-3.5" />
                        </button>
                        <button 
                    ...
                    <div className="p-4 space-y-2">
                      <span className="text-[10px] text-amber-500">{'★'.repeat(vid.rating)}</span>
                      <h4 className="font-bold text-xs text-slate-900">{vid.titleEn}</h4>
                      <p className="text-[11px] text-slate-500 leading-relaxed line-clamp-2">{vid.shortDescriptionEn}</p>
                      <div className="flex items-center space-x-2 pt-2 border-t border-slate-100 text-[10px] text-slate-400">
                        <span className="font-bold text-slate-700">{vid.clientName}</span>
                        <span>•</span>
          ...
          {activeTab === 'client-moments' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-display font-bold text-base text-slate-900">Manage Client Moments</h3>
                  <p className="text-xs text-slate-500">Edit or add real-life collaboration photos/moments with our clients</p>
                </div>
                <button
              ...
              {/* Moment Form */}
              {editingMoment && (
                <form onSubmit={handleSaveMoment} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                    <h4 className="font-bold text-sm text-slate-950">
                      {editingMoment.id ? 'Edit Client Moment' : 'Create Client Moment'}
                    </h4>
                    <button type="button" onClick={() => setEditingMoment(null)} className="text-slate-400 hover:text-slate-600 text-xs">Cancel</button>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Moment Title (English) *</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Moment Title (Bangla)</label>
                      <input 
                        type="text" value={editingMoment.titleBn || ''} 
                        onChange={e => setEditingMoment({...editingMoment, titleBn: e.target.value})}
                        className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs focus:border-blue-500 focus:outline-none"
                        placeholder="যেমন: অ্যাপেক্স ফ্যাশন টিমের সাথে হ্যান্ডওভার"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Display Order</label>
                      <input 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Description (Bangla)</label>
                      <textarea 
                        rows={2} value={editingMoment.descriptionBn || ''} 
                        onChange={e => setEditingMoment({...editingMoment, descriptionBn: e.target.value})}
                        className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs focus:border-blue-500 focus:outline-none"
                        placeholder="মুহূর্ত এবং সহযোগিতার মূল দিকগুলো বর্ণনা করুন..."
                      />
                    </div>
                  </div>

                  <div className="flex items-center space-x-2.5 text-xs font-bold text-slate-700 cursor-pointer">
                    <input 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {moments.map((mom) => (
                  <div key={mom.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm flex flex-col justify-between">
                    ...
                    <div className="p-4 space-y-2 flex-1 flex flex-col justify-between">
                      <div>
                        <h4 className="font-bold text-xs text-slate-900">{mom.titleEn}</h4>
                        <p className="text-[11px] text-slate-500 leading-relaxed line-clamp-2 mt-1">{mom.descriptionEn}</p>
                      </div>
                      <div className="flex items-center justify-between pt-3 border-t border-slate-100 text-[10px] text-slate-400 mt-2">
                        <span className="font-bold text-slate-700 truncate max-w-[120px]">{mom.clientName}</span>
          ...
          {activeTab === 'stats-logos' && (
            <div className="space-y-8">
              ...
              {/* Statistics update */}
              {stats && (
                <form onSubmit={handleSaveStats} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm">
                  <div>
                    <h3 className="font-display font-bold text-base text-slate-900">Global Trust Statistics</h3>
                    <p className="text-xs text-slate-500">Update bento statistics displayed throughout the landing view</p>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Projects Completed</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Happy Corporate Clients</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Client Satisfaction Rate (%)</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Average Star Rating</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Industries Served</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Five Star Reviews Count</label>
                      <input 
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-display font-bold text-base text-slate-900">Enterprise Client Logo Wall</h3>
                    <p className="text-xs text-slate-500">Add or remove company logos in the scrolling marquee ticker</p>
                  </div>
                  <button
                    onClick={() => setEditingLogo({})}
                    className="rounded-lg bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 text-xs font-bold transition flex items-center space-x-1"
                  >
                    <Plus className="h-4 w-4" />
                ...
                {editingLogo && (
                  <form onSubmit={handleSaveLogo} className="bg-white rounded-2xl border border-slate-200 p-4 space-y-3 shadow-sm max-w-xl">
                    <h4 className="font-bold text-xs text-slate-950">Add Brand Partner Logo</h4>
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full rounded-lg border border-slate-200 px-3 py-1 text-xs focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 mb-0.5">Logo URL (Clean vector or png) *</label>
                      <input 
                    ...
                    <div className="flex justify-end space-x-1.5 pt-1">
                      <button type="button" onClick={() => setEditingLogo(null)} className="px-3 py-1 text-xs font-bold border border-slate-200 rounded">Cancel</button>
                      <button type="submit" className="px-3 py-1 text-xs font-bold bg-blue-600 text-white rounded">Save Logo</button>
                    </div>
                  </form>
                )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                  {logos.map((logo) => (
                    <div key={logo.id} className="bg-white rounded-xl border border-slate-200 p-3 flex flex-col items-center justify-between space-y-3 hover:shadow-sm transition">
                      <img src={logo.logoUrl} alt={logo.name} className="h-8 max-w-full object-contain filter grayscale opacity-70" />
                      <div className="text-center">
                        <span className="text-[10px] font-bold text-slate-700 block">{logo.name}</span>
                        <button 
                          onClick={() => handleDeleteLogo(logo.id)}
                          className="text-[9px] font-bold text-rose-600 hover:text-rose-800 transition block mt-1"
                        >
                          Remove
                        </button>
                      </div>
          ...
          {activeTab === 'faqs' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-display font-bold text-base text-slate-900">Manage FAQs Accordion</h3>
                  <p className="text-xs text-slate-500">Edit frequently asked questions about verified review logs and calculation rules</p>
                </div>
                <button
              ...
              {/* FAQ Form */}
              {editingFaq && (
                <form onSubmit={handleSaveFaq} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm">
                  <h4 className="font-bold text-sm text-slate-950">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Category (English) *</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Category (Bangla)</label>
                      <input 
                        type="text" value={editingFaq.categoryBn || ''} 
                        onChange={e => setEditingFaq({...editingFaq, categoryBn: e.target.value})}
                        className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs focus:outline-none"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Question (English) *</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Question (Bangla)</label>
                      <input 
                        type="text" value={editingFaq.questionBn || ''} 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Answer (English) *</label>
                    <textarea 
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Answer (Bangla)</label>
                    <textarea 
                      rows={3} value={editingFaq.answerBn || ''} 
                  ...
                  <div className="flex justify-end space-x-2">
                    <button type="button" onClick={() => setEditingFaq(null)} className="px-4 py-2 border rounded text-xs font-bold bg-white">Cancel</button>
                    <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded text-xs font-bold">Save FAQ</button>
                  </div>
                </form>
              )}
                ...
                {faqs.map((faq) => (
                  <div key={faq.id} className="p-4 flex items-start justify-between hover:bg-slate-50/50 transition">
                    <div className="space-y-1 max-w-3xl">
                      <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded font-bold uppercase">{faq.categoryEn}</span>
                      <h4 className="font-bold text-slate-900 text-xs">{faq.questionEn}</h4>
                      <p className="text-slate-500 text-[11px] leading-relaxed">{faq.answerEn}</p>
                    </div>
                    <div className="flex space-x-1.5">
                      <button onClick={() => setEditingFaq(faq)} className="p-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-600 transition">
                        <Edit2 className="h-3 w-3" />
                      </button>
                      <button onClick={() => handleDeleteFaq(faq.id)} className="p-1 rounded bg-rose-50 hover:bg-rose-100 text-rose-600 transition">
          ...
          {activeTab === 'settings' && (
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
              ...
              <div className="md:col-span-6 space-y-6">
                {reviewSettings && (
                  <form onSubmit={handleSaveReviewSettings} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm">
                    <div>
                      <h3 className="font-display font-bold text-base text-slate-900">Review Gatekeeper Settings</h3>
                      <p className="text-xs text-slate-500">Configure public review submission permissions and vetting workflows</p>
                    </div>
              ...
              <div className="md:col-span-6">
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm">
                  <div>
                    <h3 className="font-display font-bold text-base text-slate-900">Landing Section Visibility</h3>
                    <p className="text-xs text-slate-500">Enable or disable any of the 14 interactive layout sections with a single click</p>
                  </div>
        ...
        {/* Footer info line */}
        <div className="bg-slate-900 border-t border-slate-800 text-slate-500 py-3 px-6 text-[10px] flex justify-between items-center">
          <span>Enterprise Secure Sync • Next Solution Bangladesh v2.6</span>
          <span className="flex items-center space-x-1">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
            <span className="text-slate-400">Database Connection Active</span>
          </span>
        </div>
